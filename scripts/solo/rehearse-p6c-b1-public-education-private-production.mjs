...
const runMarker = process.env.ATTEMPT_ID ?? `P6CB1-SMOKE-${randomUUID()}`;

if (!/^https:\/\//.test(base))
  throw new Error("COMUN_P6C_B1_PRODUCTION_HTTPS_REQUIRED");
if (!/^postgres(?:ql)?:\/\//.test(dbUrl))
  throw new Error("COMUN_P6C_B1_CURRENT_DB_SECRET_REQUIRED");
if (!/^P6CB1-SMOKE-[A-Za-z0-9._:-]{8,180}$/.test(runMarker))
  throw new Error("COMUN_P6C_B1_ATTEMPT_ID_INVALID");
...
async function request(path, init, jar) {
  ...
  if (!url.startsWith(`${base}/`)) {
    externalRequests += 1;
    throw new Error("COMUN_P6C_B1_EXTERNAL_REQUEST_REFUSED");
  }
...
async function capture(label, text, expectedSubtype, expectedPrivacy) {
  const jar = new Jar();
  ...
  assert.equal(body.receipt?.category, "public_education", `${label}:category`);
...
async function archiveExclusiveWallet(row) {
  if (
    ...
    throw new Error("COMUN_P6C_B1_CLEANUP_WALLET_NOT_EXCLUSIVELY_SYNTHETIC");
  await db.query(
    "update private.comun_participation_wallet_items set archived_at=coalesce(archived_at,now()),updated_at=now() where id=$1",
    [row.wallet_item_id],
  );
  await db.query(
    "insert into private.comun_participation_wallet_events(wallet_id,item_id,event_type,result_code) values($1,$2,'item_archived','P6CB1_SYNTHETIC_FIXTURE_CLEANUP')",
...
try {
  await capture(
    "staff",
    "Uma escola pública de teste está sem professor.",
    "staff_or_service_availability",
    "restricted",
  );
  await capture(
    "climate",
    "As salas sintéticas estão sem ventilador e muito quentes.",
    "infrastructure_or_climate",
    "restricted",
  );
  await capture(
    "meals",
    "Uma escola pública de teste está sem merenda.",
    "school_meals_or_supplies",
    "restricted",
  );
  await capture(
    "inclusion",
    "Uma unidade escolar sintética não tem rampa nem banheiro acessível.",
  ...
  for (const row of rows) {
    ...
    assert.equal(row.privacy_class, fixture.expectedPrivacy);
    assert.equal(row.retention_class, "sensitive");
    assert.equal(
      row.routing_decision.educationIssueType,
      fixture.expectedSubtype,
    );
    assert.equal(
...
console.log(
  JSON.stringify({
    result: "COMUN_P6C_B1_PUBLIC_EDUCATION_PRIVATE_WAVE1_PRODUCTION_GREEN",
