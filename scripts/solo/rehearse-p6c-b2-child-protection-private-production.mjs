...
const runMarker = process.env.ATTEMPT_ID ?? `P6CB2-SMOKE-${randomUUID()}`;
if (!/^https:\/\//.test(base))
  throw new Error("COMUN_P6C_B2_PRODUCTION_HTTPS_REQUIRED");
if (!/^postgres(?:ql)?:\/\//.test(dbUrl))
  throw new Error("COMUN_P6C_B2_CURRENT_DB_SECRET_REQUIRED");
if (!/^P6CB2-SMOKE-[A-Za-z0-9._:-]{8,180}$/.test(runMarker))
  throw new Error("COMUN_P6C_B2_ATTEMPT_ID_INVALID");
...
async function request(path, init, jar) {
  ...
  if (!url.startsWith(`${base}/`)) {
    externalRequests += 1;
    throw new Error("COMUN_P6C_B2_EXTERNAL_REQUEST_REFUSED");
  }
...
async function capture(
  ...
  assert.equal(body.receipt?.category, "child_protection", `${label}:category`);
...
async function rowsForProtocols() {
  ...
  return (
    await db.query(
      `select c.protocol,c.category,c.urgency,c.routing_decision,c.state case_state,c.withdrawn_at case_withdrawn_at,
      r.withdrawn_at report_withdrawn_at,r.retention_class,r.privacy_class,
      wi.id wallet_item_id,wi.wallet_id,wi.metadata wallet_metadata,wi.archived_at wallet_item_archived_at,w.status wallet_status,
     ...
     where c.protocol=any($1::text[]) order by c.protocol`,
...
async function archiveExclusiveWallet(row) {
  if (
    ...
    throw new Error("COMUN_P6C_B2_CLEANUP_WALLET_NOT_EXCLUSIVELY_SYNTHETIC");
  await db.query(
    "update private.comun_participation_wallet_items set archived_at=coalesce(archived_at,now()),updated_at=now() where id=$1",
    [row.wallet_item_id],
  );
  await db.query(
    "insert into private.comun_participation_wallet_events(wallet_id,item_id,event_type,result_code) values($1,$2,'item_archived','P6CB2_SYNTHETIC_FIXTURE_CLEANUP')",
...
try {
  ...
  for (const row of rows) {
    ...
    assert.equal(row.urgency, fixture.expectedUrgency);
    assert.equal(
    ...
    assert.deepEqual(row.wallet_metadata, {
      immediateDanger: fixture.expectedImmediate === true,
    });
...
console.log(
  JSON.stringify({
    result: "COMUN_P6C_B2_CHILD_PROTECTION_PRIVATE_WAVE1_PRODUCTION_GREEN",
