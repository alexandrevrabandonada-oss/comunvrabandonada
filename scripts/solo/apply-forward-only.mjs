...
if (!url || !ref || !allowed.includes(ref) || !url.includes(ref)) throw new Error("SOLO_REMOTE_DATABASE_NOT_ALLOWLISTED");
const sql = buildTransactionalPackage();
const result = spawnSync("docker", ["run", "--rm", "-i", "supabase/postgres:15.8.1.085", "psql", url, "-X", "-v", "ON_ERROR_STOP=1"], {
  input: sql,
  encoding: "utf8",
