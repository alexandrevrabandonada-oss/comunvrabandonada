...
const runMarker = process.env.ATTEMPT_ID ?? `P6BA-SMOKE-${randomUUID()}`;

if (!/^https:\/\//.test(base))
  throw new Error("COMUN_P6B_A_PRODUCTION_HTTPS_REQUIRED");
if (!/^postgres(?:ql)?:\/\//.test(dbUrl))
  throw new Error("COMUN_P6B_A_CURRENT_DB_SECRET_REQUIRED");
if (!/^P6BA-SMOKE-[A-Za-z0-9._:-]{8,180}$/.test(runMarker))
  throw new Error("COMUN_P6B_A_ATTEMPT_ID_INVALID");
...
async function request(path, init, jar) {
  ...
  if (!url.startsWith(`${base}/`)) {
    externalRequests += 1;
    throw new Error("COMUN_P6B_A_EXTERNAL_REQUEST_REFUSED");
  }
...
async function archiveExclusiveWallet(row) {
  if (
    ...
    throw new Error("COMUN_P6B_A_CLEANUP_WALLET_NOT_EXCLUSIVELY_SYNTHETIC");
  await db.query(
    "update private.comun_participation_wallet_items set archived_at=coalesce(archived_at,now()),updated_at=now() where id=$1",
    [row.wallet_item_id],
  );
  await db.query(
    "insert into private.comun_participation_wallet_events(wallet_id,item_id,event_type,result_code) values($1,$2,'item_archived','P6BA_SYNTHETIC_FIXTURE_CLEANUP')",
...
console.log(
  JSON.stringify({
    result: "COMUN_P6B_A_ENVIRONMENTAL_WAVE1_PRODUCTION_GREEN",
