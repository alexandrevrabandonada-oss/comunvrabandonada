...
const arg = process.argv.find((value) =>
  value.startsWith("--release-manifest="),
);
const ALLOWED_PUBLIC_SUMMARY_NULLABILITY =
...
const OPERATIONAL_HARDENING_RELEASE =
  "20260724233256-comun-sidewalk-operational-hardening";
const R2A_PRODUCTION_BUNDLE_RELEASE =
  "20260805130000-comun-production-pilot-core-bundle";
const R2A_ATTACHMENT_RPC_FIX_RELEASE =
...
export function selectReleaseManifest(
  ...
  if (!value) {
    ...
    const changed = execFileSync(
      "git",
      ["diff", "--name-status", `${base}...HEAD`, "--", "supabase/releases"],
      { encoding: "utf8" },
    )
      .trim()
      .split(/\r?\n/)
  ...
  if (!value.endsWith(".json") || !absolute.startsWith(`${root}${path.sep}`))
    marker("SOLO_RELEASE_MANIFEST_PATH_INVALID");
...
export function validateForwardOnlySqlText(release, migration) {
  if (
    release.destructiveSql !== false ||
    release.expectedBlockingFindings !== 0 ||
    release.platformObservationsAllowed !== true ||
    release.releaseLedger !== "public.comun_schema_releases"
  )
    marker("SOLO_CANONICAL_RELEASE_SECURITY_CONTRACT_INVALID");
  const trimmed = migration.trim();
  if (
    !/^begin;[\s\S]*commit;$/i.test(trimmed) ||
    (trimmed.match(/\bbegin;/gi) ?? []).length !== 1 ||
    (trimmed.match(/\bcommit;/gi) ?? []).length !== 1
  )
    marker("SOLO_CANONICAL_TRANSACTION_BOUNDARY_INVALID");
  ...
  if (
    ...
    (
      normalized.match(
        new RegExp(
          ALLOWED_PUBLIC_SUMMARY_NULLABILITY.replace(
            /[.*+?^${}()|[\]\\]/g,
            "\\$&",
          ),
          "g",
    ...
    marker("SOLO_PUBLIC_SUMMARY_NULLABILITY_EXCEPTION_INVALID");
  if (
    requiresLegacyGrantRepair &&
    (
      normalized.match(
        new RegExp(
          ALLOWED_LEGACY_GRANT_REPAIR.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"),
          "g",
        ),
      ) ?? []
    ).length !== 1
  )
    marker("SOLO_LEGACY_GRANT_REPAIR_EXCEPTION_INVALID");
  let remaining = allowedStatements.reduce(
  ...
  if (release.release === R2A_PRODUCTION_BUNDLE_RELEASE) {
    ...
    remaining = remaining.replace(
      /drop trigger if exists [a-z0-9_]+ on [a-z0-9_.]+;/g,
      "",
    );
    remaining = remaining
      .replace(/on delete restrict/g, "")
      .replace(/or delete on/g, "or update on");
  }
  const dynamicSql =
    release.release === R2A_PRODUCTION_BUNDLE_RELEASE ||
    release.release === R2A_ATTACHMENT_RPC_FIX_RELEASE ||
  ...
  if (
    new RegExp(
      `\\bdrop\\s+(table|schema|function|view|index|policy|sequence)\\b|\\btruncate\\b|\\bdelete\\s+from\\b|\\bcascade\\b${dynamicSql}`,
      "i",
    ).test(remaining)
  )
    marker("SOLO_CANONICAL_RELEASE_DESTRUCTIVE_SQL");
}
...
export function validateReleaseSql(manifestPath = selectReleaseManifest()) {
  ...
  if (
    !migrationPath.startsWith(
      `${path.resolve("supabase/migrations")}${path.sep}`,
    ) ||
    !existsSync(migrationPath)
  )
    marker("SOLO_RELEASE_MANIFEST_NOT_FOUND");
  const migration = readFileSync(migrationPath, "utf8");
  if (
    release.migrationSha256 !== "PENDING_LOCAL_RECONCILIATION" &&
    createHash("sha256").update(migration).digest("hex") !==
      release.migrationSha256
  )
    marker("SOLO_CANONICAL_RELEASE_CHECKSUM_MISMATCH");
...
if (
  ...
  const selected = process.argv.includes("--release-manifest")
    ? process.argv[process.argv.indexOf("--release-manifest") + 1]
    : undefined;
