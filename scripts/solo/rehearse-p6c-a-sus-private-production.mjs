...
const runMarker = process.env.ATTEMPT_ID ?? `P6CA-SMOKE-${randomUUID()}`;

if (!/^https:\/\//.test(base)) throw new Error("COMUN_P6C_A_PRODUCTION_HTTPS_REQUIRED");
if (!/^postgres(?:ql)?:\/\//.test(dbUrl)) throw new Error("COMUN_P6C_A_CURRENT_DB_SECRET_REQUIRED");
if (!/^P6CA-SMOKE-[A-Za-z0-9._:-]{8,180}$/.test(runMarker))
  throw new Error("COMUN_P6C_A_ATTEMPT_ID_INVALID");
...
async function request(path, init, jar) {
  ...
  if (!url.startsWith(`${base}/`)) {
    externalRequests += 1;
    throw new Error("COMUN_P6C_A_EXTERNAL_REQUEST_REFUSED");
  }
...
async function capture(label, text, expectedSubtype) {
  const jar = new Jar();
  const response = await request("/api/comun/relata", {
    method: "POST",
  ...
  assert.equal(body.receipt?.category, "public_health", `${label}:category`);
  assert.equal(body.noOfficialSend, true, `${label}:send_contract`);
  assert.ok(body.walletItemId, `${label}:wallet_item`);
  fixtures.push({ label, jar, protocol: body.receipt.protocol, expectedSubtype });
}

async function rowsForProtocols() {
  if (!fixtures.length) return [];
  return (await db.query(
    `select c.protocol,c.category,c.urgency,c.routing_decision,c.state case_state,
            c.withdrawn_at case_withdrawn_at,r.withdrawn_at report_withdrawn_at,
            r.retention_class,r.privacy_class,wi.id wallet_item_id,wi.wallet_id,
...
async function archiveExclusiveWallet(row) {
  if (!row.wallet_id || !row.wallet_item_id || row.wallet_item_total !== 1 || row.account_link_count !== 0)
    throw new Error("COMUN_P6C_A_CLEANUP_WALLET_NOT_EXCLUSIVELY_SYNTHETIC");
  await db.query(
    "update private.comun_participation_wallet_items set archived_at=coalesce(archived_at,now()),updated_at=now() where id=$1",
    [row.wallet_item_id],
  );
  await db.query(
    "insert into private.comun_participation_wallet_events(wallet_id,item_id,event_type,result_code) values($1,$2,'item_archived','P6CA_SYNTHETIC_FIXTURE_CLEANUP')",
...
async function softCleanup() {
  for (const fixture of fixtures) {
    const response = await request("/api/comun/relata/receipt", { method: "DELETE" }, fixture.jar);
...
async function postflight() {
  ...
  return {
    ...
    activeSyntheticPackages: rows.reduce((sum, row) => sum + Number(row.active_package_count ?? 0), 0),
    activeSyntheticAttempts: rows.reduce((sum, row) => sum + Number(row.active_attempt_count ?? 0), 0),
...
let smokeError;
try {
  await capture("staff", "Uma UBS está sem médico hoje.", "staff_or_service_availability");
  await capture("procedure", "Uma cirurgia está aguardando regulação há meses.", "exam_or_procedure");
  await capture("medicine", "Está faltando medicamento na farmácia da unidade.", "medicine_or_supply");
  await capture("facility", "O banheiro da UPA está quebrado e não tem acessibilidade.", "facility_or_accessibility");
  const rows = await rowsForProtocols();
  assert.equal(rows.length, fixtures.length);
  for (const row of rows) {
    const fixture = fixtures.find((item) => item.protocol === row.protocol);
    assert.equal(row.privacy_class, "sensitive");
    assert.equal(row.routing_decision.healthIssueType, fixture.expectedSubtype);
    ...
    assert.equal(row.active_package_count, 0);
    assert.equal(row.active_attempt_count, 0);
    assert.equal(row.snapshot_count, 0);
  }
} catch (error) {
  smokeError = error;
} finally {
...
console.log(JSON.stringify({
  result: "COMUN_P6C_A_SUS_PRIVATE_WAVE1_PRODUCTION_GREEN",
