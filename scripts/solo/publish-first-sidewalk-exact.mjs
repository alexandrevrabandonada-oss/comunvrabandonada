...
const artifactDir = path.resolve(
  process.env.COMUN_ARTIFACT_DIR ??
    ".ci-artifacts/sidewalk-first-exact-publication",
);
...
try {
  ...
  const candidates = await db.query(candidateSql);
  evidence.candidateCountBefore = candidates.rowCount;
  const already = await db.query(alreadyPublishedSql, [publicSummary]);
  ...
  const locked = await db.query(lockCandidateSql, [candidate.id]);
  if (locked.rowCount !== 1) throw new Error("CANDIDATE_CHANGED_BEFORE_COMMIT");
  ...
  evidence.cycleResult = committed
    ? "COMUN_SIDEWALK_FIRST_EXACT_PUBLICATION_CRITICAL_POSTFLIGHT"
...
async function publicSmoke(slug) {
  const [mapResponse, recordResponse] = await Promise.all([
    fetch(`${baseUrl}/comun/calcadas`, { redirect: "follow" }),
  ...
  if (mapResponse.status !== 200 || recordResponse.status !== 200)
    throw new Error("PUBLIC_SMOKE_NOT_GREEN");
  const recordHtml = await recordResponse.text();
  if (!recordHtml.includes(publicSummary))
    throw new Error("PUBLIC_SUMMARY_NOT_VISIBLE");
}
