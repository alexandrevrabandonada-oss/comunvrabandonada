...
import { resourceNames, runIndependentRound, TARGET_RELEASE } from "./reconcile-sidewalk-scoped.mjs";

const MANIFEST = "supabase/releases/20260724233256-comun-sidewalk-operational-hardening.json";
const manifestHash = () => createHash("sha256").update(readFileSync(MANIFEST)).digest("hex");
const originalManifestHash = manifestHash();
const allowedResultFields = ["cleanupComplete", "constraintsValid", "ledgerStatus", "migrationChecksum", "post", "pre", "roundId", "sidewalkForeignKeys", "structuralCounts"];

class FakeCommandRunner {
  constructor({ constraints = "0", foreignKeys = "6", preLedger = "0", postLedger = "applied", failAt } = {}) {
  ...
  async run(command, args = [], options = {}) {
    ...
    if (this.failAt === command) throw new Error(`synthetic ${command} failure`);
    if (command === "psql") {
      const sql = args.at(-1);
    ...
    if (command === "capture-scoped-fingerprint") return args.at(-1) === "pre" ? "synthetic-pre" : "synthetic-post";
    return "";
  }
}
...
const cleanupCalls = (runner) => runner.calls.filter(({ command, args }) => command === "docker" && args.includes("rm"));
const assertManifestIntact = () => assert.equal(manifestHash(), originalManifestHash);
const assertCleanup = (runner) => {
  assert.deepEqual(cleanupCalls(runner).map(({ args }) => args.slice(0, 2)), [["rm", "-f"], ["volume", "rm"], ["network", "rm"]]);
};
...
test("resources are exclusive for every roundId", async () => {
  ...
  assert.notEqual(firstRunner.calls[0].args.at(-1), secondRunner.calls[0].args.at(-1));
  assertManifestIntact();
});
...
test("invalid constraints block before PRE and migration and still clean up", async () => {
  ...
  await assert.rejects(() => runIndependentRound(options(runner)), /COMUN_SCOPED_ROUND_PRECONDITION_FAILED/);
  assert.equal(commandNames(runner).includes("capture-scoped-fingerprint"), false);
...
test("five foreign keys block, while six permit advancement to migration", async () => {
  ...
  await assert.rejects(() => runIndependentRound(options(blocked)), /COMUN_SCOPED_ROUND_PRECONDITION_FAILED/);
...
test("absent or divergent POST ledgers block after migration and clean up", async () => {
  for (const postLedger of ["", "divergent"]) {
    ...
    await assert.rejects(() => runIndependentRound(options(runner, `post-${postLedger || "absent"}`)), /COMUN_SCOPED_ROUND_LEDGER_INVALID/);
...
test("cleanup runs after restore and migration failures", async () => {
  for (const failAt of ["restore-schema", "apply-local-migration"]) {
    ...
    await assert.rejects(() => runIndependentRound(options(runner, `failure-${failAt}`)), new RegExp(`synthetic ${failAt} failure`));
    assertCleanup(runner);
  }
  assertManifestIntact();
});
...
test("remote contexts are rejected before commands and rejection details are sanitized", async () => {
  ...
  await assert.rejects(
    () => runIndependentRound({ ...options(runner), projectRef: "remote", dbUrl: dsn, token, backupDir: backupPath }),
    (error) => {
      assert.match(error.message, /^SOLO_SCOPED_RECONCILIATION_REMOTE_FORBIDDEN$/);
      assert.doesNotMatch(error.message, /operator|private-token|remote\.example|Users|backup/i);
      return true;
    },
  );
...
test("all executor operations are fake commandRunner calls with no real Docker, PostgreSQL, Supabase, or network invocation", async () => {
  ...
  assert.equal(TARGET_RELEASE, "20260724233256-comun-sidewalk-operational-hardening");
  assertManifestIntact();
});
