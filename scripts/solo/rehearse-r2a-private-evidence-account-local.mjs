...
async function waitForServer() {
  for (let i = 0; i < 90; i++) {
    if (server.exitCode !== null) {
      throw new Error(
        `COMUN_R2A_LOCAL_SERVER_EXIT_${server.exitCode}\n${output.join("")}`,
      );
    }
    try {
      const response = await fetch(`${base}/comun/minha-participacao`);
...
const server = spawn(
  ...
  ["run", serverScript, "--", "-p", new URL(base).port],
  {
    cwd: process.cwd(),
    env: process.env,
...
try {
  ...
  const wrongReceiptCookie = receiptCookie
    .split(";")
    .map((part) =>
      part.trim().startsWith("comun_relata_receipt_v1=")
        ? "comun_relata_receipt_v1=invalid"
        : part.trim(),
    )
    .join("; ");
  assert.notEqual(wrongReceiptCookie, receiptCookie);
  const wrongLocation = await httpWithCookie(
  ...
  if (started.status === 404) {
    ...
    try {
      ...
      await diagnostic.query(
        ...
        [protocol, receiptSecret, randomUUID(), "image/png", "under_1mb"],
      );
  ...
  const invalidType = await http("/api/comun/relata/evidence/attachments", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ mimeType: "image/gif", sizeBytes: png.byteLength }),
  });
  assert.equal(invalidType.status, 400);
  const oversize = await http("/api/comun/relata/evidence/attachments", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
      mimeType: "image/png",
  ...
  const privateDownload = await fetch(
    `${base}/api/comun/relata/evidence/attachments/${attachmentId}`,
  );
  assert.equal(privateDownload.status, 404);
  const secondWallet = await fetch(`${base}/api/comun/participation-wallet`, {
    method: "POST",
  });
  assert.equal(secondWallet.status, 201);
  const secondWalletCookie = (
    secondWallet.headers.get("set-cookie") ?? ""
  ).split(";", 1)[0];
  assert.match(secondWalletCookie, /^comun_participation_wallet_v1=/);
  const crossWallet = await fetch(
    `${base}/api/comun/relata/evidence/attachments/${attachmentId}`,
    { headers: { cookie: secondWalletCookie } },
  );
  assert.equal(crossWallet.status, 404);
  const isolatedWallet = await fetch(`${base}/api/comun/participation-wallet`, {
    headers: { cookie: secondWalletCookie },
  });
  ...
  const authResponse = await fetch(
    `${process.env.NEXT_PUBLIC_SUPABASE_URL}/auth/v1/signup`,
    {
      ...
      body: JSON.stringify({ email: authEmail, password: `${token()}Aa1!` }),
    },
  );
  ...
  const walletToken = cookie.match(
    /comun_participation_wallet_v1=([^;]+)/,
  )?.[1];
  assert.ok(walletToken);
  const cryptoHash = (value) =>
    createHash("sha256").update(`comun-wallet-v1:${value}`).digest("hex");
  const linked = await client.query(
    "select * from public.comun_participation_wallet_link_account($1,$2,'explicit_account_link')",
    [cryptoHash(walletToken), authUser.user.id],
  );
  assert.equal(linked.rows[0].linked, true);
  const linkRow = await client.query(
    "select count(*)::int as count from private.comun_participation_wallet_account_links where user_id=$1",
    [authUser.user.id],
  );
  assert.equal(linkRow.rows[0].count, 1);
  const evidenceRow = await client.query(
    "select octet_length(encrypted_value)::int as encrypted_bytes, octet_length(nonce)::int as nonce_bytes, evidence_state from private.comun_relata_private_locations where report_id=(select report_id from public.comun_relata_cases where protocol=$1)",
    [protocol],
  );
  ...
  const attachmentRow = await client.query(
    "select state, review_required_for_publication from private.comun_relata_attachments where id=$1",
    [attachmentId],
  );
  assert.deepEqual(attachmentRow.rows[0], {
    state: "sealed_private",
    review_required_for_publication: true,
  });
  await client.end();

  const wrongRecovery = await fetch(
    `${base}/api/comun/participation-wallet/recovery/redeem`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ recoveryCode: "AAAA-AAAA-AAAA-AAAA-AAAA-AAAA" }),
    },
  );
  ...
  const recovered = await fetch(
    `${base}/api/comun/participation-wallet/recovery/redeem`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ recoveryCode: createdBody.walletRecoveryCode }),
    },
  );
  assert.equal(recovered.status, 200);
  assert.equal((await recovered.json()).recovered, true);

  const removedPhoto = await httpWithCookie(
    `/api/comun/relata/evidence/attachments/${attachmentId}`,
    receiptCookie,
    { method: "DELETE" },
  );
  assert.equal(removedPhoto.status, 200);
  const removedLocation = await httpWithCookie(
    "/api/comun/relata/evidence/location",
    receiptCookie,
    { method: "DELETE" },
  );
  ...
  console.log(
    JSON.stringify({
      result: "COMUN_48_1B_R2A_PRIVATE_EVIDENCE_ACCOUNT_E2E_GREEN",
      location: "encrypted_server_only",
      photo: "sealed_private_derivative",
      wallet: "http_cookie_item",
      account: "local_auth_explicit_link",
      collective: "deferred_disabled",
  ...
  if (output.length && process.env.COMUN_R2A_E2E_LOG) {
    writeFileSync(process.env.COMUN_R2A_E2E_LOG, output.join(""), {
      flag: "a",
    });
  }
}
