...
export const MIGRATION_LANE_MANIFEST = Object.freeze({
  ...
  "20260823003249_comun_cultural_specialized_provenance_readiness.sql":
    "culture-a5-a1",
  "20260824001340_comun_artwork_submission_private_materialization.sql":
    "culture-a5-a2",
  ...
  "20260826090000_comun_denuncias_public_collective_projection.sql":
    "culture-b0",
  "20260826120000_comun_denuncias_public_projection_opt_in.sql": "culture-b1",
  "20260826150000_comun_denuncias_public_evidence_pauta_bridge.sql":
    "culture-b2-a1",
  "20260827120000_comun_denuncias_private_collective_matching.sql":
    "culture-b2-a2",
  ...
  "20260926153200_comun_relata_collective_entity_public_projection.sql":
    "collective-entity-public-projection",
});
...
const NON_APPLICABLE_LANES = Object.freeze({
  ...
  "collective-entity-private-candidate": new Set(
    Object.values(MIGRATION_LANE_MANIFEST).filter(
      (owner) => owner !== "collective-entity-private-candidate",
    ),
  ),
  "collective-entity-legitimacy-eligibility": new Set(
    Object.values(MIGRATION_LANE_MANIFEST).filter(
      (owner) => owner !== "collective-entity-legitimacy-eligibility",
    ),
  ),
  "collective-entity-public-projection": new Set(
    Object.values(MIGRATION_LANE_MANIFEST).filter(
      (owner) => owner !== "collective-entity-public-projection",
    ),
  ),
});
...
for (const lanes of Object.values(NON_APPLICABLE_LANES)) {
  ...
  lanes.add("collective-entity-public-projection");
...
function parseArgs(argv) {
  ...
  if (laneIndex < 0 || !argv[laneIndex + 1])
    throw new Error("--lane is required");
  const filesIndex = argv.indexOf("--files");
  return {
    lane: argv[laneIndex + 1],
    files: filesIndex < 0 ? [] : argv.slice(filesIndex + 1),
  };
}

if (process.argv[1]?.endsWith("classify-migration-lane.mjs")) {
  try {
    const result = classifyMigrationLane(
      ...Object.values(parseArgs(process.argv.slice(2))),
    );
    console.log(`COMUN_MIGRATION_LANE_MODE=${result.mode}`);
    console.log(`COMUN_MIGRATION_LANE=${result.lane}`);
    if (result.reason)
      console.error(`COMUN_MIGRATION_LANE_REASON=${result.reason}`);
