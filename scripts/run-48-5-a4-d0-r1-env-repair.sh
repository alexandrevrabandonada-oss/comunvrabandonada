...
trap 'rm -f "$PROJECT_JSON" "$SHARED_JSON" "$TEAM_JSON" "$ENV_FILE" "$PATCH_BODY" "$PATCH_RESPONSE" "$PATCH_STATUS"' EXIT
...
curl -sS -o "$PATCH_RESPONSE" -w '%{http_code}' -X PATCH -H "Authorization: Bearer $VERCEL_TOKEN" -H 'Content-Type: application/json' --data-binary "@$PATCH_BODY" "https://api.vercel.com/v9/projects/$VERCEL_PROJECT_ID/env/$ENV_ID?teamId=$VERCEL_ORG_ID" > "$PATCH_STATUS"
node --input-type=module - "$PATCH_STATUS" "$PATCH_RESPONSE" "$ARTIFACT_DIR/a4-d0-r1-patch-receipt.json" <<'NODE'
import fs from 'node:fs';
...
const receipt = sanitizePatchResult({ status, payload });
