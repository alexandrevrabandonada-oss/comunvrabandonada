...
for (const name of ["SUPABASE_DB_URL", "SUPABASE_ACCESS_TOKEN", "SUPABASE_SERVICE_ROLE_KEY"])
  if (Object.hasOwn(process.env, name)) throw new Error("COMUN_R3_PRODUCTION_SECRET_PRESENT");
...
try {
  ...
  `)).rows;
  const publicRelations = (await db.query(`
  ...
  `)).rows[0].count;
  const expectedColumns = ["id","entity_id","generation_request_id","source_representation_id",
    "source_consent_id","candidate_version","candidate_state","public_name",
    "entity_type","generated_at","invalidated_at","invalidation_reason"];
  const expectedTriggers = ["comun_relata_candidate_consent_invalidate",
    "comun_relata_candidate_entity_invalidate","comun_relata_candidate_immutable",
    "comun_relata_candidate_representation_invalidate"];
  if (!row || row.read_only !== "on" || !row.rls || !row.force_rls || row.owner !== "postgres" ||
      row.anon_select || row.authenticated_select || row.service_select ||
      row.anon_insert || row.authenticated_insert || row.service_insert ||
      JSON.stringify(columns) !== JSON.stringify(expectedColumns) ||
      indexes.length !== 1 || indexes[0].unique_index !== true ||
      !indexes[0].predicate?.includes("pending_legitimacy") ||
      JSON.stringify(triggers) !== JSON.stringify(expectedTriggers) ||
      bridge.length !== 1 || !bridge[0].security_definer || bridge[0].owner !== "postgres" ||
      !bridge[0].config?.includes("search_path=pg_catalog") ||
      bridge[0].anon_execute || bridge[0].authenticated_execute ||
      bridge[0].public_execute || !bridge[0].service_execute ||
      publicRelations !== 0 || migrationCount !== 1)
