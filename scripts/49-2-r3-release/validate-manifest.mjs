...
export const MANIFEST_PATH = "supabase/release-bundles/20260924-comun-49-2-r3-private-candidate.json";
export const BASELINE_PATH = "reports/current/comun-49-2-r3-production-pre.json";
export const DERIVED_PATH = "reports/current/comun-49-2-r3-release-disposable-derived.json";
const sha256 = (value) => createHash("sha256").update(value).digest("hex");

export function validateR3Manifest({ manifest, baseline, derived, baselineBytes,
  derivedBytes, migrationBytes }) {
  const migration = manifest?.migrations?.[0];
  const expectedPath = "supabase/migrations/20260924225210_comun_relata_collective_entity_private_candidate.sql";
  const migrationSetHash = sha256(JSON.stringify([[expectedPath, migration?.sha256]]));
  if (manifest?.release !== "20260924-comun-49-2-r3-private-candidate" ||
      manifest?.sourceMainSha !== "c47ee61bb2011dc5139f4b673e94c3557737c6cb" ||
      manifest.migrations?.length !== 1 || migration?.order !== 1 ||
      migration.version !== "20260924225210" || migration.path !== expectedPath ||
      ...
      baseline.transactionReadOnly !== "on" || baseline.blockingFindings !== 0 ||
      baseline.r1Present !== true || baseline.r2Present !== true || baseline.r3Present !== false ||
...
export function loadValidatedR3Manifest() {
  ...
  return validateR3Manifest({ manifest, baseline, derived, baselineBytes,
    derivedBytes, migrationBytes });
}
