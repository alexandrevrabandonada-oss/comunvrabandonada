...
export function requireR3WriteAuthorization({
  authorization,
  expectedSha,
  disposable,
}) {
...
export function createR3PostgresAdapter({
  url,
  manifest,
  disposable = false,
  authorization,
  ...
  const capture = async () => {
    ...
    try {
      ...
      `,
          [[r12.release, hardening.release, manifest.release]],
        )
      ).rows;
      return {
  ...
  const verifyPost = async () => {
    ...
    try {
      ...
      if (mode !== "on") throw new Error("COMUN_R3_POST_VERIFY_NOT_READ_ONLY");
      const table = (
        await db.query(`
      ...
      if (
        ...
        index.length !== 1 ||
        !index[0].unique_index ||
        !index[0].predicate?.includes("pending_legitimacy") ||
        triggerCount !== 4
      )
        throw new Error("COMUN_R3_POST_STRUCTURE_INVALID");
      return {
        status: "COMUN_R3_POST_STRUCTURE_ACCEPTED",
