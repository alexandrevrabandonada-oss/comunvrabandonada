...
node scripts/49-2-r3-release/verify-r3-structure.mjs --output="$artifact/structure.json"
...
# Independently rehearse the forward-only runner on a fresh Production-like PRE.
database="${database}2"
docker exec -e PGPASSWORD=postgres "$container" createdb -U supabase_admin "$database"
for file in post-schema.sql technical-ledger.sql synthetic-buckets.sql restore-expression.sql; do
  docker exec -e PGPASSWORD=postgres "$container" psql -U supabase_admin -d "$database" \
...
node scripts/49-2-r3-release/run-disposable-promotion.mjs --output="$artifact/runner.json"
