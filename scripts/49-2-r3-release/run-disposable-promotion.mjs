...
for (const name of [
  ...
  if (Object.hasOwn(process.env, name))
    throw new Error("COMUN_R3_DISPOSABLE_PRODUCTION_SECRET_PRESENT");
const url = requireDisposableUrl(process.env.COMUN_DISPOSABLE_DB_URL ?? "");
const adminUrl = requireDisposableUrl(
...
const result = await promoteR3Release({
  manifest,
  baseline,
  ...adapter,
  applyMigration,
...
const replay = await promoteR3Release({
  manifest,
  baseline,
  ...adapter,
  applyMigration,
...
writeFileSync(
  ...
  `${JSON.stringify(
    {
      status: "COMUN_49_2_R3_FORWARD_ONLY_REHEARSAL_GREEN",
