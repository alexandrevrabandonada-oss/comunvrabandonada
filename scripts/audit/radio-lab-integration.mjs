...
async function checked(query) {
  const r = await query;
  if (r.error)
    throw new Error(`Local database error ${r.error.code ?? "unknown"}`);
  return r.data;
}
async function test(name, fn) {
  ...
  fs.writeFileSync(
    report,
    JSON.stringify(
      { sha: command("git", ["rev-parse", "HEAD"]).trim(), run, results },
      null,
      2,
    ),
  );
...
async function actor(role) {
  const email = `audit-${role}-${run}@example.invalid`,
  ...
  if (role === "editor" || role === "viewer")
    await checked(
      db
        ...
        .insert({ user_id: data.user.id, email, role, is_active: true }),
    );
  const cookies = new Map();
  const auth = createServerClient(env.API_URL, env.ANON_KEY, {
    global: { fetch: guardedFetch },
    cookies: {
...
try {
  ...
  while (true) {
    try {
      const r = await fetch(base + "/comun/admin/login");
      if (r.status === 200) break;
    } catch {
      /* startup */
    }
    if (Date.now() > deadline || app.exitCode !== null)
      throw new Error("Local application failed readiness");
    await new Promise((resolve) => setTimeout(resolve, 200)); // readiness only; not a concurrency barrier
  }
  const A = await actor("editor"),
    B = await actor("ordinary"),
    V = await actor("viewer");
  const program = crypto.randomUUID();
  ids.push(program);
  await checked(
    db.from("comun_archive_items").insert({
      id: program,
      slug: `audit-program-${run}`,
  ...
  await checked(
    db.from("comun_radio_programs").insert({
      archive_item_id: program,
      title_public: "Synthetic program",
      slug_public: `audit-program-${run}`,
      description_public: "Disposable laboratory",
      format_type: "mixed",
    }),
  );
  ...
  await checked(
    db.from("comun_radio_episodes").insert({
      archive_item_id: episode,
      program_item_id: program,
      title_public: "Synthetic radio audit",
      slug_public: `audit-${run}`,
      summary_public: "Synthetic only",
      description_public: "Disposable laboratory",
      transcript_status: "unavailable",
    }),
  );
  await checked(
    db.from("comun_radio_voice_consents").insert({
      episode_item_id: episode,
      participant_reference_private: `private-${run}`,
  ...
  const grants = JSON.parse(
    sql(
      "select json_agg(json_build_object('role',r,'asset_select',has_table_privilege(r,'public.comun_archive_assets','select'),'radio_select',has_table_privilege(r,'public.comun_radio_episodes','select'))) from unnest(array['anon','authenticated','service_role']) r;",
    ),
  );
  results.push({
    name: "grant matrix (catalog, separate from RLS)",
    status: "PASS",
    evidence: grants,
  });
  const policies = sql(
    "select json_agg(json_build_object('table',tablename,'policy',policyname,'roles',roles,'command',cmd)) from pg_policies where schemaname in ('public','storage') and tablename in ('comun_archive_assets','comun_radio_episodes','objects');",
  );
  results.push({
    name: "RLS policy catalog",
    status: "PASS",
    evidence: JSON.parse(policies),
  });
  ...
  await test("unauthorized and viewer requests cannot forge editor identity", async () => {
    for (const cookie of ["", B.cookie, V.cookie]) {
      const r = await request(
        "/api/comun/admin/archive/confirm-upload",
        cookie,
        { assetId: episode, userId: A.id, role: "admin" },
      );
  ...
  const prepared = await request(
    "/api/comun/admin/archive/upload-url",
    A.cookie,
    {
  ...
  const row = await checked(
    db.from("comun_archive_assets").select("*").eq("id", assetId).single(),
  );
  objectKeys.push(["radio-private-originals", row.object_key]);
  const upload = await guardedFetch(prepared.json.uploadUrl, {
    method: "PUT",
  ...
  await test("application upload persisted canonical private metadata and real bytes", async () => {
    assert.equal(row.bucket_scope, "private_original");
    assert.equal(row.public_url, null);
    const info = await db.storage
      .from("radio-private-originals")
      .info(row.object_key);
    assert.equal(info.error, null);
    assert.equal(info.data.size, wav.length);
    assert.equal(info.data.contentType, "audio/wav");
  });
  await test("B and anonymous cannot download or sign original; authorized signature works", async () => {
    for (const c of [B.auth, anon]) {
      ...
      assert.ok(
        (
          await c.storage
            .from("radio-private-originals")
            .createSignedUrl(row.object_key, 30)
        ).error,
      );
    }
    ...
    assert.ok((await guardedFetch(signed.data.signedUrl)).ok);
  ...
  await test("B cannot confirm A original via swapped asset ID", async () => {
    assert.equal(
      (
        await request("/api/comun/admin/archive/confirm-upload", B.cookie, {
          assetId,
        })
      ).status,
      401,
  ...
  await test("real confirmation and replay after discarded response", async () => {
    ...
    assert.equal(
      (
        await request("/api/comun/admin/archive/confirm-upload", A.cookie, {
          assetId,
        })
      ).status,
      200,
    ...
    assert.equal(
      (
        await checked(
          db.from("comun_archive_assets").select("id").eq("id", assetId),
        )
      ).length,
      1,
    );
  ...
  await test("timeout while confirmation waits for database commit can be retried", async () => {
    assertOwned(s);
    lock = new pg.Client({ connectionString: env.DB_URL });
    await lock.connect();
    await lock.query("begin");
    await lock.query(
      "select id from comun_archive_assets where id=$1 for update",
      [assetId],
    );
    let socket;
    const timed = request(
    ...
    const observed = timed.then(
      () => false,
      (e) => e.message === "AUDIT_SYNTHETIC_TIMEOUT",
    );
    const deadline = Date.now() + 30000;
    while (true) {
      const waiting = await lock.query(
        "select count(*)::int n from pg_stat_activity where wait_event_type='Lock' and pid<>pg_backend_pid()",
      );
      if (waiting.rows[0].n >= 1) break;
      if (Date.now() > deadline)
        throw new Error("Timeout case did not reach database barrier");
  ...
  await test("simultaneous confirmations: independent HTTP + deterministic database row barrier", async () => {
    ...
    const first = request("/api/comun/admin/archive/confirm-upload", A.cookie, {
      assetId,
    });
    ...
    while (true) {
      const q = await lock.query(
        "select count(*)::int n from pg_stat_activity where wait_event_type='Lock' and pid<>pg_backend_pid()",
      );
      if (q.rows[0].n >= 1) break;
      if (Date.now() > limit)
        throw new Error("Independent requests did not reach database barrier");
      await new Promise(setImmediate);
    }
    await lock.query("commit");
    await lock.end();
  ...
  function clearFault() {
    sql(
      `drop trigger if exists ${faultName} on public.comun_archive_assets; drop function if exists public.${faultName}();`,
    );
  }
  ...
  await test("real FFmpeg processing persists MP3 and waveform without editorial publication", async () => {
    const response = await request("/api/comun/admin/radio/process", A.cookie, {
      episodeId: episode,
      assetId,
    });
    assert.equal(response.status, 200);
    const assets = await checked(
      db
        .from("comun_archive_assets")
        .select("asset_role,bucket_scope,object_key")
        .eq("archive_item_id", episode)
        .in("asset_role", ["radio_public_episode", "radio_waveform"]),
    );
    assert.equal(assets.length, 2);
    for (const a of assets) {
      objectKeys.push(["radio-public-audio", a.object_key]);
      assert.equal(a.bucket_scope, "public_safe");
      assert.equal(
        (await db.storage.from("radio-public-audio").download(a.object_key))
          .error,
        null,
      );
    }
    const e = await checked(
      db
        .from("comun_radio_episodes")
  ...
  await test("failure after FFmpeg before metadata persistence resumes explicitly", async () => {
    fault(
      "insert",
      `NEW.archive_item_id='${episode}'::uuid and NEW.asset_role in ('radio_public_episode','radio_waveform')`,
    );
    try {
      assert.notEqual(
        (
          await request("/api/comun/admin/radio/process", A.cookie, {
            episodeId: episode,
            assetId,
          })
        ).status,
      ...
      const physical = await db.storage
        .from("radio-public-audio")
        .info(`radio-public/${episode}/episode.mp3`);
  ...
  await test("withdrawal while an old worker runs cannot republish derivatives", async () => {
    assertOwned(s);
    lock = new pg.Client({ connectionString: env.DB_URL });
    await lock.connect();
    await lock.query("begin");
    await lock.query("lock table comun_admin_audit_log in share mode");
    const processing = request("/api/comun/admin/radio/process", A.cookie, {
      episodeId: episode,
      assetId,
    });
    const deadline = Date.now() + 30000;
    while (true) {
      ...
      if (waiting.rows[0].n >= 1) break;
      if (Date.now() > deadline)
        throw new Error("Old worker did not reach the deterministic barrier");
      await new Promise(setImmediate);
    }
    await checked(
      db
    ...
    await lock.end();
    lock = null;
    assert.equal((await processing).status, 409);
    const rows = await checked(
      db
        .from("comun_archive_assets")
        .select("id")
    ...
    for (const key of [
      `radio-public/${episode}/episode.mp3`,
      `radio-public/${episode}/waveform.json`,
    ])
      assert.ok((await db.storage.from("radio-public-audio").info(key)).error);
    await checked(
      db
        .from("comun_radio_episodes")
    ...
    return "State changed while the worker was blocked; final compare-and-set rejected publication and removed both derivatives";
  });
  await test("two processing requests at database barrier; B remains unauthorized during processing", async () => {
    ...
    await lock.query("begin");
    await lock.query("lock table comun_admin_audit_log in share mode");
    const first = request("/api/comun/admin/radio/process", A.cookie, {
      episodeId: episode,
      assetId,
    });
    const second = request("/api/comun/admin/radio/process", A.cookie, {
      episodeId: episode,
      assetId,
    });
    const deadline = Date.now() + 30000;
    while (true) {
      ...
      if (waiting.rows[0].n >= 1) break;
      if (Date.now() > deadline)
        throw new Error("Processing request did not reach barrier");
      await new Promise(setImmediate);
    }
    assert.equal(
      (
        await request("/api/comun/admin/radio/process", B.cookie, {
          episodeId: episode,
          assetId,
          userId: A.id,
        })
    ...
    assert.equal(rows.length, 2);
    return "Independent HTTP/TCP requests; one worker claimed processing and the competing request was rejected";
  });
  await test("unpublished HTTP projection does not expose original or private reference", async () => {
    ...
    assert.ok(
      !body.includes(row.object_key) && !body.includes(`private-${run}`),
    );
  });
} catch (e) {
  results.push({
    name: "integration setup or required dependency",
    status: "FAIL",
    reason: e.name,
    detail: String(e.message)
      .replace(/https?:\/\/\S+/g, "[url]")
  ...
  await test("cleanup owned fixtures only", async () => {
    ...
    for (const id of [...ids].reverse()) {
      const rows = await checked(
        db
          .from("comun_archive_assets")
          .select("object_key,bucket_scope")
          .eq("archive_item_id", id),
      );
      for (const r of rows)
        if (
          r.object_key.startsWith(`radio-originals/${id}/`) ||
          r.object_key.startsWith(`radio-public/${id}/`)
        )
          objectKeys.push([
            r.bucket_scope === "private_original"
    ...
    for (const [bucket, key] of objectKeys) {
      const r = await db.storage.from(bucket).remove([key]);
      assert.equal(r.error, null);
    }
    for (const id of users) {
      const admins = await checked(
        db.from("comun_admin_users").select("id").eq("user_id", id),
      );
      ...
      await checked(db.from("comun_admin_users").delete().eq("user_id", id));
  ...
  if (app?.pid && app.exitCode === null) {
    try {
      if (process.platform === "win32")
        command("taskkill", ["/PID", String(app.pid), "/T", "/F"]);
      else app.kill("SIGTERM");
    } catch {
      results.push({
        name: "owned application process cleanup",
        status: "FAIL",
      });
    }
  }
  ...
  console.log(
    JSON.stringify(results.map(({ name, status }) => ({ name, status }))),
  );
