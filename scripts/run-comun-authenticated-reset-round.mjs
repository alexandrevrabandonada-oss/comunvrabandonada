...
async function rebuild(){const started=Date.now();const result=spawnSync("node",["scripts/comun-local-env.mjs","run","npx","supabase","db","reset","--local"],{cwd:process.cwd(),env,encoding:"utf8",shell:process.platform==="win32",stdio:["ignore","pipe","pipe"],maxBuffer:64*1024*1024});const migrations=appliedMigrations();const recovery=await waitForLocalSupabaseRecovery();records.push({label:"reset",ok:result.status===0||migrations===expectedMigrations,durationMs:Date.now()-started,exitCode:result.status,migrations,expectedMigrations,recovered:recovery.ok,recoveryDurationMs:recovery.durationMs});if(migrations!==expectedMigrations)throw new Error(`migrations incompletas: ${migrations}/${expectedMigrations}`);if(result.status!==0)console.log(`COMUN_RESET_TRANSIENT_RECOVERED exitCode=${result.status} migrations=${migrations}`)}
