...
const appliedMigrations=()=>{const result=spawnSync("docker",["exec","supabase_db_COMUM_VR_ABANDONADA","psql","-U","postgres","-d","postgres","-tAc","select count(*) from supabase_migrations.schema_migrations"],{cwd:process.cwd(),encoding:"utf8"});if(result.status!==0)throw new Error(`consulta de migrations falhou: ${result.stderr.trim()}`);return Number(result.stdout.trim())};
