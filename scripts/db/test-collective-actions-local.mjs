...
try {
  ...
  sql(
    `update public.comun_collective_actions set status = 'completed', completed_at = now(), result_summary = 'Resultado confirmado.', memory_summary = 'Aprendizado preservado.' where id = '${ids.action}'`,
  );
  const memory = sql(
