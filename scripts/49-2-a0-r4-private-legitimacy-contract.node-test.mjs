...
test("R4 eligibility is derived and never mutates the R3 candidate into an approval state", () => {
  ...
  assert.doesNotMatch(
    migration,
    /sets+candidate_states*=s*'(?:verified|approved|eligible|published)'/i,
  );
  assert.doesNotMatch(
    migration,
    /updates+private.comun_relata_collective_entity_candidates[sS]*eligible_for_projection_review/i,
  );
});
...
test("R4 reviewer identity is server-derived and browser inputs cannot choose authority", () => {
  ...
  assert.match(
    migration,
    /profile.role in ('admin','factual_reviewer','editorial_reviewer')/,
  );
