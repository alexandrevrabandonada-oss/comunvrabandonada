...
do $$
declare r record; replay record; route text; p text; h text; t text;
begin
  ...
  foreach t in array array['comun_archive_submissions','comun_archive_artwork_submissions','comun_archive_oral_history_suggestions','comun_radio_contributions'] loop
    if not has_table_privilege('service_role','public.'||t,'INSERT') then
      raise exception 'service_role cannot create specialized target %', t;
    end if;
    if has_table_privilege('anon','public.'||t,'INSERT') or has_table_privilege('authenticated','public.'||t,'INSERT') then
      raise exception 'client can create specialized target %', t;
    end if;
  end loop;
  ...
  foreach route in array array['photo_or_document','art','oral_history','radio','unknown'] loop
    ...
    insert into a3_intakes(route_kind,protocol,resume_hash) values(route,r.public_protocol,h);
    p := r.public_protocol;
    ...
    if route='unknown' then
      ...
      update a3_intakes set target_kind = r.target_kind where route_kind = route;
...
begin
  ...
  foreach route in array array['photo_or_document','art','oral_history','radio'] loop
    select target_kind into t from a3_intakes where route_kind=route;
    if t is null then raise exception 'target kind not recorded for %', route; end if;
  end loop;
  foreach t in array array['comun_archive_submissions','comun_archive_artwork_submissions','comun_archive_oral_history_suggestions','comun_radio_contributions'] loop
    ...
    if after_count <> before_count + 1 then raise exception 'target count is not exactly one for %',t; end if;
  end loop;
  foreach t in array array['comun_archive_items','comun_archive_assets','comun_archive_collections','comun_archive_collection_items','comun_archive_relations','comun_hub_archive_links','comun_search_documents'] loop
    execute format('select row_count from a3_baseline where table_name=%L',t) into before_count;
    execute format('select count(*) from public.%I',t) into after_count;
    if after_count <> before_count then raise exception 'forbidden public projection write for %',t; end if;
  end loop;
end $$;
...
select 'COMUN_48_5_A3_DISPOSABLE_SPECIALIZED_HANDOFF_GREEN' as checkpoint,
