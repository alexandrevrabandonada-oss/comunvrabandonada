...
import { createR4PostgresAdapter, requireDisposableUrl } from "./postgres-adapter.mjs";
...
for (const name of ["SUPABASE_DB_URL","SUPABASE_ACCESS_TOKEN","SUPABASE_SERVICE_ROLE_KEY"])
  if (Object.hasOwn(process.env, name))
    throw new Error("COMUN_R4_DISPOSABLE_PRODUCTION_SECRET_PRESENT");
...
if (new URL(url).pathname !== new URL(adminUrl).pathname)
  throw new Error("COMUN_R4_DISPOSABLE_DATABASE_MISMATCH");

const { manifest, baseline } = loadValidatedR4Manifest();
const adapter = createR4PostgresAdapter({ url, manifest, disposable: true });
...
const result = await promoteR4Release({ manifest, baseline, ...adapter, applyMigration });
if (result.state !== "POST" || result.actions.join(",") !== "R4,LEDGER")
  throw new Error("COMUN_R4_DISPOSABLE_FORWARD_ONLY_INCOMPLETE");

const replay = await promoteR4Release({ manifest, baseline, ...adapter, applyMigration });
if (replay.state !== "POST" || replay.actions.length !== 0)
  throw new Error("COMUN_R4_DISPOSABLE_REPLAY_INVALID");

const output = process.argv.find((arg) => arg.startsWith("--output="))?.slice(9);
if (!output) throw new Error("COMUN_R4_DISPOSABLE_OUTPUT_MISSING");
writeFileSync(output, `${JSON.stringify({
  status:"COMUN_49_2_R4_FORWARD_ONLY_REHEARSAL_GREEN",
...
console.log("COMUN_49_2_R4_FORWARD_ONLY_REHEARSAL_GREEN");
