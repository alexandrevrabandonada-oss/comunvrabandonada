...
case "$MODE" in flags-off|wave1-organization-onboarding) ;; *) exit 1;; esac
...
set_flag(){ local value="$1"; run_vercel bash -c 'printf "%s" "$1" | npx --yes vercel@50.28.0 env add COMUN_SOLIDARITY_ORGANIZATION_ONBOARDING_ENABLED production --force --yes --token "$2" --scope "$3"' -- "$value" "$VERCEL_TOKEN" "$VERCEL_ORG_ID"; }
...
rollback(){ set +e; set_flag disabled; deploy; set -e; echo COMUN_48_4_A4_VERCEL_ROLLBACK_GREEN >> "$GITHUB_STEP_SUMMARY"; }
...
if [ "$MODE" = flags-off ]; then
  test "$(status GET /comun/cooperativas/nova)" = 404
  test "$(status HEAD /comun/cooperativas/nova)" = 404
else
  test "$(status GET /comun/cooperativas/nova)" = 200
  test "$(status HEAD /comun/cooperativas/nova)" = 200
  page="$(mktemp)"; remember "$page"; curl -L -fsS --retry 8 --retry-delay 2 "$COMUN_BASE_URL/comun/cooperativas/nova" >"$page"
  grep -q 'Qual organização você quer incluir?' "$page"
  ! grep -Eqi 'applicant_user_id|participation_note_private|review_message_private|request_id|private_contact|transition_history_private' "$page"
fi
if [ "$MODE" = flags-off ]; then
  echo COMUN_48_4_A4_FLAGS_OFF_PRODUCTION_GREEN >> "$GITHUB_STEP_SUMMARY"
else
  echo COMUN_48_4_A4_WAVE1_PRODUCTION_GREEN >> "$GITHUB_STEP_SUMMARY"
fi
