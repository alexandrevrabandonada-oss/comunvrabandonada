...
test("novo visitante conclui primeira participação e retorna à pauta",async({page},testInfo)=>{
  ...
  await page.setInputFiles('input[name="photo"]',".local/comun-integral/calcada-fixture.jpg");await page.getByRole("button",{name:"Continuar"}).click();await page.getByRole("button",{name:"Mapa local para marcar o ponto"}).click({position:{x:180,y:120}});await page.getByLabel("Bairro ou referência pública").fill("Centro, trecho sintético");await page.getByRole("button",{name:"Continuar"}).click();await page.getByText("Ruim",{exact:true}).click();await page.getByLabel("Problema principal").selectOption("irregular");await page.getByLabel("Descrição opcional").fill("Trecho sintético com piso irregular e risco de queda durante o teste local.");await page.getByRole("button",{name:"Continuar"}).click();await noCritical(page);await page.screenshot({path:`reports/screenshots/sprint-37-integral-revisao-${testInfo.project.name}.png`,fullPage:true});
  await page.getByRole("button",{name:/Enviar contribuição/}).click();await expect(page.getByRole("heading",{name:/Recebemos seu registro/})).toBeVisible();await noCritical(page);await page.screenshot({path:`reports/screenshots/sprint-34-2-confirmacao-${testInfo.project.name}.png`,fullPage:true});
