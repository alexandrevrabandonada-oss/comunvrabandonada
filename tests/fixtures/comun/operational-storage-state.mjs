...
export async function validateOperationalStorageState({
  ...
  if (!normalizedPath.includes(`/${runId}/`)) return invalidate(path, "run id incompatível");
  const cookies = state?.cookies ?? [];
  const authCookies = cookies.filter((cookie) => /^sb-.*auth-token/i.test(cookie.name) && Boolean(cookie.value));
  if (!authCookies.length) return invalidate(path, "cookie de sessão ausente");
  if (authCookies.some(isExpired)) return invalidate(path, "cookie de sessão expirado");

  const context = await browser.newContext({ storageState: path });
  try {
    const page = await context.newPage();
    await page.goto(new URL(protectedPath, baseUrl).toString());
    if (new URL(page.url()).pathname.includes("/login") || new URL(page.url()).pathname.includes("/entrar")) {
      return invalidate(path, "redirect para login");
    }
    await page.getByRole("heading", { name: heading }).waitFor({ state: "visible" });
    if (await page.getByLabel("E-mail").count()) return invalidate(path, "formulário de login presente");
    if (identityEmail) {
      await page.goto(new URL("/comun/admin/acervo", baseUrl).toString());
      await page.getByText(identityEmail, { exact: true }).waitFor({ state: "visible" });
    }
    const currentCookies = await context.cookies();
    if (!currentCookies.some((cookie) => /^sb-.*auth-token/i.test(cookie.name) && Boolean(cookie.value) && !isExpired(cookie))) {
