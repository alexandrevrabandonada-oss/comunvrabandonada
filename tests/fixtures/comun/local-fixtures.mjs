...
function localServiceConfig() {
  ...
  const raw =
    process.platform === "win32"
      ? execFileSync(
          "powershell",
          ["-NoProfile", "-Command", "npx supabase status -o env"],
          {
            encoding: "utf8",
  ...
  if (
    ...
    !env.API_URL?.match(/^https?:\/\/(localhost|127\.0\.0\.1)/)
  )
    throw new Error("Configuração local do Supabase ausente.");
  return {
    url: process.env.NEXT_PUBLIC_SUPABASE_URL || env.API_URL,
...
export function localPublicConfig() {
  const config = localServiceConfig();
  return {
    url: config.url,
    anonKey: process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || config.anonKey,
  };
}
...
export async function createLocalAuthFixtures() {
  ...
  for (const [persona, email] of Object.entries(localPersonaEmails)) {
    ...
    const { data, error } = await db.auth.admin.createUser({
      ...
      email_confirm: true,
      user_metadata: { fixture: "s28-2", persona },
    });
    if (error || !data.user)
      throw new Error(
        `auth fixture ${persona}: ${error?.message ?? "sem usuário"}`,
      );
  ...
  await db
    ...
    .upsert(
      {
        user_id: adminId,
        email: localPersonaEmails.admin,
        role: "admin",
        is_active: true,
      },
      { onConflict: "email" },
  ...
  await db.from("comun_member_profiles").upsert(
    Object.entries(users).map(([persona, user_id]) => ({
      user_id,
      display_name: `Persona ${persona}`,
...
export async function createLocalPautaMiniappFixture(existingUsers) {
  ...
  const { data: pauta, error } = await db
    .from("comun_pauta_spaces")
    .insert({
      slug: tag,
      title: "Pauta pública de fixture",
      summary: "Resumo estável para smoke local.",
      visibility: "public",
      public_synthesis: "Síntese estável da fixture.",
      next_step: "Próxima ação de teste.",
    })
    .select("id,slug")
    .single();
  if (error || !pauta)
    throw new Error(error?.message ?? "Pauta fixture não criada.");
  await db.from("comun_pauta_modules").insert([
    {
      pauta_id: pauta.id,
      module_type: "overview",
      title_override: "Visão geral de teste",
      status: "active",
      visibility: "public",
      position: 0,
      config: {},
    ...
    {
      pauta_id: pauta.id,
      module_type: "construction_circle",
      title_override: "Roda pública de teste",
      status: "active",
      visibility: "public",
      position: 1,
      config: {},
  ...
  const { data: circle, error: circleError } = await db
    ...
    .insert({
      pauta_id: pauta.id,
      title: "Roda de escuta fixture",
      public_question: "O que deve acontecer agora?",
      status: "open",
    })
    .select("id")
    .single();
  if (circleError || !circle)
    throw new Error(circleError?.message ?? "Roda fixture não criada.");
  const { data: round, error: roundError } = await db
    ...
    .insert({
      ...
      title: "Escuta aberta",
      public_prompt: "Compartilhe uma contribuição segura.",
      position: 0,
      status: "open",
    })
    .select("id")
    .single();
  if (roundError || !round)
    throw new Error(roundError?.message ?? "Rodada fixture não criada.");
  await db
    .from("comun_construction_circles")
    .update({ current_round_id: round.id })
    .eq("id", circle.id);
  await db
    .from("comun_circle_contributions")
    .insert({
      circle_id: circle.id,
      round_id: round.id,
      contribution_type: "testimony",
      public_body: "Contribuição pública e moderada da fixture.",
      status: "visible",
      public_protocol: `RODA-${tag}`,
    });
  await db
    ...
    .insert({
      circle_id: circle.id,
      round_id: round.id,
      public_summary: "Síntese publicada da fixture.",
      agreements: ["Acordo de teste"],
  ...
  await db.from("comun_pauta_memberships").insert([
    {
      pauta_id: pauta.id,
      member_user_id: users.facilitator,
      role: "facilitator",
      status: "active",
    },
    {
      pauta_id: pauta.id,
      member_user_id: users.participant,
      role: "participant",
      status: "active",
    },
  ]);
...
export async function cleanupLocalComunFixtures() {
  const db = localServiceClient();
  const { data: pautas } = await db
    .from("comun_pauta_spaces")
    .select("id")
    .like("slug", `${prefix}%`);
  for (const pauta of pautas ?? [])
    await db.from("comun_pauta_spaces").delete().eq("id", pauta.id);
...
export async function assertNoComunTestFixtures() {
  ...
  const authCount =
    auth?.users.filter((user) =>
      Object.values(localPersonaEmails).includes(user.email),
    ).length ?? 0;
  if (error || count || authCount)
    throw new Error(
      `Fixtures COMUN restantes: pautas=${count ?? "erro"}, auth=${authCount}`,
    );
