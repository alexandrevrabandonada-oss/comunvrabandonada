...
test("visitante entende navegacao finita", async ({ page }) => {
  ...
  await expect(page.getByText("Entenda o momento", { exact: false })).toBeVisible();
...
test("busca preserva origem e não usa popularidade", async ({ page }) => {
  ...
  await expect(page.locator("body")).not.toContainText(/curtidas|seguidores|popularidade:/i);
});

test("@visual screenshots públicas", async ({ page }, testInfo) => {
  for (const [name, route] of [["home", "/comun"], ["participar", "/comun/participar"], ["busca", "/comun/buscar?q=territorio"], ["territorios", "/comun/territorios"], ["radio", "/comun/radio"]]) {
    await page.goto(route);
    await page.screenshot({ path: `reports/screenshots/sprint-31-${name}-${testInfo.project.name}.png`, fullPage: true });
  }
});
