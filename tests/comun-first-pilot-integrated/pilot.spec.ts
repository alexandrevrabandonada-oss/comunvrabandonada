...
test("visitante percorre território comunidade pauta resultado e memória sem conta", async ({ page }, testInfo) => {
  const { slug } = JSON.parse(await readFile(".comun-sidewalk-pilot-slug", "utf8"));
  await page.goto("/comun");
  await expect(page.getByRole("heading", { name: /Organize seu território/ })).toBeVisible();
...
test("pessoa nova acompanha contribui consulta inbox e sai", async ({ page }, testInfo) => {
  ...
  await page.getByRole("button", { name: /Concluir e voltar à comunidade/ }).click();
  ...
  await page.goto("/comun");
  await expect(page.getByRole("heading", { name: "Nas suas comunidades" })).toBeVisible();
  await page.goto(`/comun/pautas/${slug}`);
  ...
  await expect(page.getByRole("heading", { name: "Grupos de trabalho" })).toBeVisible();
