...
returns boolean language plpgsql security definer set search_path=pg_catalog,private,public as $$ declare w uuid; begin select id into w from private.comun_participation_wallets where token_hash=decode(p_token_hash_hex,'hex') and status='active'; if w is null then return false; end if; update private.comun_participation_wallet_account_links set revoked_at=coalesce(revoked_at,now()) where wallet_id=w and user_id=p_user_id and revoked_at is null; if found then insert into private.comun_participation_wallet_events(wallet_id,event_type,result_code) values(w,'item_updated','WALLET_ACCOUNT_LINK_REVOKED'); end if; return found; end; $$;
...
create or replace function public.comun_relata_get_evidence_state(p_protocol text,p_receipt_secret text)
returns table(evidence jsonb) language plpgsql stable security definer set search_path=pg_catalog,private,public as $$
declare x record; location_state text := 'not_added'; location_approximation text := 'none'; photos jsonb := '[]'::jsonb;
begin
  select * into x from private.comun_relata_authorized_context(p_protocol,p_receipt_secret);
  if not found then return; end if;
  select case when l.evidence_state='withdrawn' then 'withdrawn' when l.approximation_level in ('neighborhood','region') then 'approximate_private' else 'added_private' end,
         case when l.approximation_level in ('neighborhood','region') then l.approximation_level else 'none' end
    into location_state,location_approximation
    from private.comun_relata_private_locations l where l.report_id=x.report_id;
  select coalesce(jsonb_agg(jsonb_build_object(
    'label','Foto '||a.label_index,
    'state',a.state,
    'mimeType',a.actual_mime_type,
    'width',a.width,
    'height',a.height,
    'reviewRequiredForPublication',true,
    'accessUrl','/api/comun/relata/evidence/attachments/'||a.id::text
  ) order by a.label_index),'[]'::jsonb) into photos
    from private.comun_relata_attachments a where a.report_id=x.report_id;
  return query select jsonb_build_object(
    'location',location_state,
    'locationApproximation',location_approximation,
    'photos',photos,
    'grouping','case_individual',
    'groupingConfidence','blocked',
    'activeReportsInCollective',0,
    'noOfficialSend',true,
...
create or replace function public.comun_relata_authorize_attachment_read(p_protocol text,p_receipt_secret text,p_attachment_id uuid) returns table(attachment_id uuid,label_index smallint,attachment_state text,object_key text,derivative_object_key text) language sql stable security definer set search_path=pg_catalog,private,public as $$ select a.id,a.label_index,a.state,a.object_key,a.derivative_object_key from private.comun_relata_attachments a where a.id=p_attachment_id and a.report_id=(select report_id from private.comun_relata_authorized_context(p_protocol,p_receipt_secret)) and a.state='sealed_private'; $$;
create or replace function public.comun_relata_reject_attachment(p_protocol text,p_receipt_secret text,p_attachment_id uuid,p_rejection_code text) returns boolean language plpgsql security definer set search_path=pg_catalog,private,public as $$ begin update private.comun_relata_attachments a set state='rejected',rejection_code=left(p_rejection_code,80),updated_at=now() where a.id=p_attachment_id and a.report_id=(select report_id from private.comun_relata_authorized_context(p_protocol,p_receipt_secret)) and a.state in ('quarantine','validating'); return found; end; $$;
...
do $$ declare s text[] := array[
 ...
 'public.comun_relata_add_location(text,text,text,text,timestamptz,bytea,bytea,bytea,text,text,text,text)','public.comun_relata_withdraw_location(text,text)','public.comun_relata_get_evidence_state(text,text)',
 'public.comun_relata_begin_attachment(text,text,uuid,text,text)','public.comun_relata_mark_attachment_validating(text,text,uuid)','public.comun_relata_finalize_attachment(text,text,uuid,text,bigint,bigint,integer,integer,bytea,bytea)','public.comun_relata_authorize_attachment_read(text,text,uuid)','public.comun_relata_reject_attachment(text,text,uuid,text)','public.comun_relata_withdraw_attachment(text,text,uuid)',
 'public.comun_participation_wallet_create(text,text)','public.comun_participation_wallet_list(text)','public.comun_participation_wallet_attach_relata(text,text,text)','public.comun_participation_wallet_follow_legacy(text,text)','public.comun_participation_wallet_follow_case(text,text,text)','public.comun_participation_wallet_claim_bus(text,text,jsonb)','public.comun_participation_wallet_remove_item(text,uuid)','public.comun_participation_wallet_redeem(text,text)','public.comun_participation_wallet_rotate_recovery(text,text,text)','public.comun_participation_wallet_link_account(text,uuid,text)','public.comun_participation_wallet_revoke_account(text,uuid)']; f text; begin foreach f in array s loop execute 'revoke all on function '||f||' from public,anon,authenticated'; execute 'grant execute on function '||f||' to service_role'; end loop; end $$;
