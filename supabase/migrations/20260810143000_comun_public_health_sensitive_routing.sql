...
begin
  ...
  if jsonb_typeof(coalesce(p_answers,'{}'::jsonb)) <> 'object'
    or coalesce(p_answers,'{}'::jsonb)
      ...
      - 'health_issue_type' - 'blocked' - 'line' - 'direction' - 'unit' - 'school_type' <> '{}'::jsonb
...
begin
  ...
  update private.comun_relata_reports set
    ...
    privacy_class=case when p_category='public_health' then v_privacy_class else privacy_class end,
    retention_class=case when p_category='public_health' then 'sensitive' else retention_class end,
    review_after=case when p_category='public_health' then now()+interval '30 days' else review_after end,
    updated_at=now()
