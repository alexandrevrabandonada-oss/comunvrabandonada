...
alter table private.comun_relata_reports
  alter column original_text drop not null;

alter table private.comun_relata_reports
  drop constraint comun_relata_reports_original_text_check;

alter table private.comun_relata_reports
  add constraint comun_relata_reports_original_text_semantics_check
  check (
    original_text is null
    or char_length(trim(original_text)) between 8 and 600
  );
