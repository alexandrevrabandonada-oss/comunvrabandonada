...
alter table private.comun_relata_classification_events
  ...
  check (next_category in (
    ...
    'urban_flooding','stormwater_drainage','tree_hazard','public_health','public_education'
  ));
...
begin
  ...
  if jsonb_typeof(coalesce(p_answers,'{}'::jsonb)) <> 'object'
    or coalesce(p_answers,'{}'::jsonb)
      ...
      - 'health_issue_type' - 'education_issue_type' - 'blocked' - 'line' - 'direction' - 'unit' - 'school_type' <> '{}'::jsonb
  ...
  insert into private.comun_relata_reports(
    ...
    case when p_category='public_education' or p_privacy_class in ('sensitive','high_risk') then 'sensitive' else 'private_unsubmitted' end,
    now()+case when p_category='public_education' or p_privacy_class in ('sensitive','high_risk') then interval '30 days' else interval '90 days' end
...
begin
  ...
  update private.comun_relata_reports set
    ...
    routing_rule_version='relata-routing-v1',urgency=v_urgency,
    privacy_class=case when p_category in ('public_health','public_education') then v_privacy_class else privacy_class end,
    retention_class=case when p_category in ('public_health','public_education') then 'sensitive' else retention_class end,
    review_after=case when p_category in ('public_health','public_education') then now()+interval '30 days' else review_after end,
    updated_at=now()
  ...
  update private.comun_participation_wallet_items set
    ...
    metadata=case when p_category='public_education'
      then jsonb_build_object(
        'educationIssueType',v_education_issue_type,
        'childSafetySignal',v_child_safety_signal
      )
...
begin
  ...
  insert into private.comun_participation_wallet_items(
    ...
    'Relato COMUN',c.category,'Guardado','Precisa de informação',
