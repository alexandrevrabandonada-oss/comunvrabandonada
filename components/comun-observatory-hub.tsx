...
import type { SidewalkCoverageState } from "@/lib/comun-sidewalk-observatory";
...
export function ObservatorySourceDisclosure({
  source,
}: {
  ...
  return (
    <details className="mt-5 border-t-2 border-comun-black/15 pt-3 text-sm">
      ...
      <dl className="mt-3 grid gap-3 sm:grid-cols-2">
        ...
        <Meta
          label="Período"
          value={source.observedPeriod ?? "Período por registro revisado"}
        />
      ...
      {stale ? (
        <p className="mt-3 border-l-4 border-comun-yellow pl-3 font-bold">
          Este dado pode estar desatualizado.
        </p>
      ) : null}
      <p className="mt-3 text-comun-black/70">
        Limitação: esta leitura não representa todas as calçadas da cidade e não
        inclui relatos privados.
      </p>
    </details>
  );
}
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8 sm:py-12">
      <header className="max-w-3xl">
        ...
        <h1 className="mt-2 text-4xl font-black uppercase tracking-[-.04em] text-comun-paper sm:text-6xl">
          Observatórios
        </h1>
        <p className="mt-4 text-lg text-comun-paper/80">
          Dados públicos e informações revisadas para entender o território.
        </p>
      </header>
      <section
      ...
      >
        {observatories.map((item) => {
          ...
          return (
            ...
            >
              <p
                className={`text-xs font-black uppercase ${available ? "text-comun-rust" : "text-comun-black/60"}`}
              >
              ...
              {sidewalk && available && sidewalkCount !== null ? (
                <p className="mt-4 text-sm font-bold">
                  {partial ? "Mais de " : ""}
                  {sidewalkCount} ponto(s) revisado(s) na projeção pública.
                </p>
              ) : null}
              {available ? (
                <Link
                  href={item.publicRoute!}
                  className="mt-5 inline-flex min-h-11 items-center bg-comun-yellow px-4 font-black uppercase text-comun-black"
                >
                  Ver observatório
                </Link>
              ) : null}
              {sidewalk && sidewalkSource && available ? (
                <ObservatorySourceDisclosure source={sidewalkSource} />
              ) : null}
            </article>
          );
        })}
      ...
      >
        ...
        <p className="mt-2">
          O COMUN reúne somente fontes já públicas, projeções comunitárias
          revisadas ou dados editoriais com proveniência. Relatos privados,
          fotos, localizações, Carteira e dados de Saúde, Educação ou Proteção
...
function Meta({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <dd className="mt-1 text-comun-black/75">{value}</dd>
    </div>
  );
}
