...
import { ComunMemberNavigation, ComunMobileNavigation, ComunPrimaryNavigation } from "./comun-navigation";
import { ComunPwaRuntime, ComunShareButton } from "./comun-pwa-runtime";

export function ComunAppShell({ children }: { children: ReactNode }) {
  return <div className="min-h-screen overflow-x-hidden pb-[env(safe-area-inset-bottom)]">
    <ComunPwaRuntime />
    <a href="#conteudo" className="sr-only z-50 bg-comun-yellow p-3 font-black text-comun-black focus:not-sr-only focus:fixed focus:left-2 focus:top-2">Pular para o conteúdo</a>
    <div role="note" className="bg-comun-yellow px-4 py-1 text-center text-xs font-black text-comun-black">Ambiente de demonstração · conteúdo sintético</div>
    <header className="sticky top-0 z-30 border-b-2 border-comun-yellow bg-comun-black pt-[env(safe-area-inset-top)] text-comun-paper">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-2">
        <Link href="/comun" aria-label="COMUN VR Abandonada" className="max-w-32 text-lg font-black leading-none tracking-[-.05em] text-comun-paper">COMUN<span className="text-comun-yellow">.</span></Link>
        <ComunPrimaryNavigation />
        <div className="flex items-center gap-1 sm:gap-2"><ComunMemberNavigation /><ComunShareButton title="COMUN VR Abandonada" /><ParticipateSheet /><SearchSheet /></div>
      </div>
    </header>
    <main id="conteudo" className="pb-24 md:pb-0">{children}</main>
