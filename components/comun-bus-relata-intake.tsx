...
export function ComunBusRelataIntake({ attachmentsEnabled, locationEnabled }: { attachmentsEnabled: boolean; locationEnabled: boolean }) {
  ...
  const [issueType, setIssueType] = useState<ComunBusIssueType | null>(null);
  ...
  return <main className="mx-auto grid max-w-2xl gap-6 px-4 py-8" data-comun-bus-p5="intake">
    ...
    <fieldset className="grid gap-3 border-2 bg-white p-4"><legend className="px-1 text-xl font-black">O que aconteceu?</legend><div className="grid gap-2 sm:grid-cols-2">{COMUN_BUS_ISSUE_TYPES.map((value) => <button key={value} type="button" aria-pressed={issueType === value} onClick={() => setIssueType(value)} className={`min-h-11 border-2 px-3 py-2 text-left font-bold ${issueType === value ? "bg-comun-yellow" : "bg-white"}`}>{COMUN_BUS_ISSUE_LABELS[value]}</button>)}</div></fieldset>
    <details className="border-2 bg-white" open={detailsOpen} onToggle={(event) => setDetailsOpen(event.currentTarget.open)}>
      <summary aria-expanded={detailsOpen} className="flex min-h-12 cursor-pointer items-center px-4 py-3 text-xl font-black focus-visible:outline focus-visible:outline-4 focus-visible:outline-comun-yellow">Adicionar detalhes (opcional)</summary>
      <section className="grid gap-3 border-t-2 p-4" aria-label="Detalhes opcionais do problema no ônibus">
    ...
    <button type="button" disabled={busy || !issueType} onClick={save} className="min-h-12 border-2 bg-comun-yellow px-5 py-3 font-black shadow-[4px_4px_0_#0b0b0a] disabled:cursor-not-allowed disabled:opacity-50">{busy ? "Guardando…" : "Guardar"}</button>
    {!issueType ? <p className="text-sm font-bold">Escolha o que aconteceu para guardar.</p> : null}
