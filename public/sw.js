...
const PRIVATE_PREFIXES = [
  "/comun/admin", "/comun/minha-participacao", "/comun/caixa-de-entrada", "/comun/conta",
  "/comun/entrar", "/comun/criar-conta", "/comun/onboarding", "/comun/mapa/contribuir", "/api/",
];

function isPublicPage(url) {
  return url.origin === self.location.origin && url.pathname.startsWith("/comun/") &&
...
self.addEventListener("activate", (event) => {
  event.waitUntil(Promise.all([
    caches.keys().then((keys) => Promise.all(keys.filter((key) => key.startsWith("comun-pwa-") && ![SHELL_CACHE, PUBLIC_CACHE].includes(key)).map((key) => caches.delete(key)))),
    self.clients.claim(),
  ]));
});
...
self.addEventListener("message", (event) => {
  ...
  if (event.data?.type === "CLEAR_PRIVATE") {
    event.waitUntil(caches.keys().then((keys) => Promise.all(keys.filter((key) => key.includes("private")).map((key) => caches.delete(key)))));
  }
});
...
self.addEventListener("fetch", (event) => {
  ...
  if (request.mode === "navigate" && isPublicPage(url)) {
    event.respondWith(fetch(request).then(async (response) => {
      if (canStore(response)) (await caches.open(PUBLIC_CACHE)).put(request, response.clone());
      return response;
    }).catch(async () => (await caches.match(request)) || (await caches.match("/comun/offline"))));
    return;
  }
  if (request.mode === "navigate" && url.pathname.startsWith("/comun")) {
    event.respondWith(fetch(request).catch(() => caches.match("/comun/offline")));
    return;
  }
  if (url.origin === self.location.origin && (url.pathname.startsWith("/_next/static/") || url.pathname.startsWith("/icons/"))) {
    event.respondWith(caches.match(request).then((cached) => cached || fetch(request).then(async (response) => {
      if (canStore(response)) (await caches.open(SHELL_CACHE)).put(request, response.clone());
      return response;
    })));
  }
});
