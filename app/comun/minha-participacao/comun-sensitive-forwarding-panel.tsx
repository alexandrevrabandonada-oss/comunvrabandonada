...
export function ComunSensitiveForwardingPanel({
  ...
  const [channelOnly, setChannelOnly] = useState(category === "child_protection");
  const [disclosure, setDisclosure] =
    useState<SensitiveDisclosureInput>(EMPTY_DISCLOSURE);
  const [preview, setPreview] = useState<SensitiveDisclosurePreview | null>(null);
  const [authorizationProof, setAuthorizationProof] = useState<string | null>(null);
  ...
  if (!pkg) {
    return (
      <section className="mt-3 grid gap-3 border-t-2 pt-3" data-comun-sensitive-forwarding="review">
        <header>
          <p className="font-black">
            {channelOnly ? "Canais de proteção disponíveis" : "Quer abrir um canal oficial?"}
          </p>
          <p className="text-sm">
            {channelOnly
        ...
        {!channelOnly ? (
          <fieldset className="grid gap-3 border-2 bg-white p-3">
            <legend className="px-1 font-black">O que você quer levar para este canal?</legend>
            {issueLabel ? (
              <Check
            ...
            <DisclosureField
              label={category === "public_health" ? "Nome da unidade" : "Escola ou unidade"}
            ...
            <label className="grid gap-2 font-bold">
              <Check
                ...
                onChange={(value) => update("includePersonAuthoredSummary", value)}
              ...
              <textarea
                ...
                onChange={(event) => update("personAuthoredSummary", event.target.value)}
                maxLength={1000}
        ...
        {notice ? <p role="status" className="border-l-4 border-comun-yellow pl-3 text-sm font-bold">{notice}</p> : null}
      </section>
    );
  }

  const prepared = pkg.attempts?.filter((attempt) => attempt.state === "prepared") ?? [];
  const sent = pkg.attempts?.filter((attempt) =>
    ["person_declared_sent", "no_response"].includes(attempt.state),
  ) ?? [];
  return (
    <section className="mt-3 grid gap-3 border-t-2 pt-3" data-comun-sensitive-forwarding={channelOnly ? "channel-only" : "minimal"}>
      <header>
        <p className="font-black">Encaminhamento preparado</p>
        <p className="text-sm">Nada foi enviado. Somente você pode abrir o canal e concluir a manifestação.</p>
      </header>
      {!channelOnly ? (
        <div className="grid gap-2 border-2 bg-[#f8f2e6] p-3">
          ...
          <button type="button" onClick={() => navigator.clipboard?.writeText(pkg.institutional_text)} className="min-h-11 w-fit font-black underline">
            Copiar mensagem
          </button>
        </div>
      ) : (
        <p className="border-l-4 border-comun-yellow pl-3 text-sm font-bold">
          Nenhum conteúdo da situação será copiado pelo COMUN. Conte diretamente ao canal.
        </p>
      )}
      <div className="grid gap-2">
        {channels.map((channel) => (
          <article key={channel.id} className="grid gap-2 border-2 p-3">
            ...
            <button type="button" disabled={busy} onClick={() => openChannel(channel)} className="btn w-fit bg-comun-yellow">
              {channel.channelType === "phone" ? "Abrir telefone" : "Continuar para o canal"}
            </button>
            <p className="text-xs">Fonte oficial verificada; funcionamento não testado. {channel.notes}</p>
            {channel.sourceUrls.map((url) => (
              <a key={url} href={url} target="_blank" rel="noreferrer" className="w-fit text-xs font-bold underline">Consultar fonte oficial</a>
            ))}
          </article>
        ))}
      </div>
      ...
      {sent.map((attempt) => (
        <div key={attempt.attemptId} className="grid gap-2 border-2 p-3">
          <label className="grid gap-1 text-sm font-bold">
            ...
            <select value={outcome} onChange={(event) => setOutcome(event.target.value)} className="min-h-11 border-2 p-2 font-normal">
              ...
              <option value="prefer_not_to_record_details">Prefiro não registrar detalhes</option>
            </select>
          </label>
          {!channelOnly ? (
            <label className="grid gap-1 text-sm font-bold">Nota curta opcional
              <textarea value={responseNote} onChange={(event) => setResponseNote(event.target.value)} maxLength={280} className="border-2 p-2 font-normal" />
            </label>
          ) : null}
          <label className="grid gap-1 text-sm font-bold">Protocolo informado por você (opcional e não verificado)
            <input value={officialProtocol} onChange={(event) => setOfficialProtocol(event.target.value)} maxLength={240} className="border-2 p-2 font-normal" />
          </label>
          <button type="button" disabled={busy} onClick={() => recordResponse(attempt.attemptId)} className="btn w-fit">Registrar andamento</button>
        </div>
      ))}
      <button type="button" disabled={busy} onClick={withdraw} className="min-h-11 w-fit font-black underline">Retirar encaminhamento e seu conteúdo</button>
      {notice ? <p role="status" className="border-l-4 border-comun-yellow pl-3 text-sm font-bold">{notice}</p> : null}
    </section>
  );
}
...
function Check({ checked, onChange, label }: { checked: boolean; onChange: (value: boolean) => void; label: string }) {
  return <label className="flex min-h-11 items-center gap-2 font-bold"><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} className="size-5" />{label}</label>;
}

function DisclosureField({ label, checked, value, maxLength, onChecked, onValue }: { label: string; checked: boolean; value: string; maxLength: number; onChecked: (value: boolean) => void; onValue: (value: string) => void }) {
  return <div className="grid gap-1"><Check checked={checked} onChange={onChecked} label={label} /><input value={value} disabled={!checked} onChange={(event) => onValue(event.target.value)} maxLength={maxLength} className="min-h-11 border-2 p-2 disabled:opacity-50" /></div>;
}

function Preview({ value, onConfirm, busy }: { value: SensitiveDisclosurePreview; onConfirm: () => void; busy: boolean }) {
  return <section className="grid gap-3 border-2 bg-white p-3" aria-label="Revisão do compartilhamento">
    <div><p className="font-black">SERÁ COMPARTILHADO</p><ul className="list-disc pl-5 text-sm">{value.sharedItems.map((item) => <li key={item}>{item}</li>)}</ul></div>
    <div><p className="font-black">NÃO SERÁ COMPARTILHADO</p><ul className="list-disc pl-5 text-sm">{value.notSharedItems.map((item) => <li key={item}>{item}</li>)}</ul></div>
    {value.institutionalText ? <pre className="whitespace-pre-wrap border-l-4 border-comun-yellow pl-3 text-sm">{value.institutionalText}</pre> : null}
    <button type="button" disabled={busy} onClick={onConfirm} className="btn w-fit bg-comun-yellow">Continuar para o canal</button>
  </section>;
}
