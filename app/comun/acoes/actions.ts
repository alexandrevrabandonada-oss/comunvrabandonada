...
async function memberAction(form: FormData) {
  ...
  const { data: action } = await db.from("comun_collective_actions").select("id,status,visibility").eq("slug", slug).eq("visibility", "public").in("status", ["open", "active", "awaiting_result"]).maybeSingle();
...
export async function updateCollectiveActionParticipation(form: FormData) {
  ...
  if (!participationStates.has(status)) throw new Error("Forma de participação inválida.");
  const note = value(form, "contribution_note_private").slice(0, 600);
  const { error } = await db.from("comun_collective_action_participations").upsert({
    action_id: action.id,
...
export async function claimCollectiveActionTask(form: FormData) {
  ...
  const { data: task } = await db.from("comun_collective_action_tasks").select("id").eq("id", taskId).eq("action_id", action.id).in("state", ["open", "in_progress"]).maybeSingle();
  if (!task) throw new Error("Esta tarefa não está disponível.");
  const { error } = await db.from("comun_collective_action_task_assignments").upsert({
    task_id: task.id,
    member_user_id: userId,
    status: "active",
    released_at: null,
  ...
  if (error) {
    if (error.message?.includes("COMUN_COLLECTIVE_TASK_CAPACITY_REACHED")) throw new Error("As vagas desta tarefa já foram preenchidas.");
...
export async function releaseCollectiveActionTask(form: FormData) {
  const { slug, userId, db } = await memberAction(form);
  const taskId = value(form, "task_id");
  if (!taskId) throw new Error("Tarefa inválida.");
  const { error } = await db.from("comun_collective_action_task_assignments").update({ status: "released", released_at: new Date().toISOString() }).eq("task_id", taskId).eq("member_user_id", userId).eq("status", "active");
