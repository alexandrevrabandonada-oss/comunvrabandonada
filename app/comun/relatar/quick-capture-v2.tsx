...
const RelataEvidencePanel = dynamic(
  () => import("@/app/comun/relata/relata-evidence-panel").then((module) => module.RelataEvidencePanel),
  { ssr: false },
);
...
export function QuickCaptureV2({ attachmentsEnabled = false, locationEnabled = false }: { attachmentsEnabled?: boolean; locationEnabled?: boolean }) {
    ...
    <ComunShell showSyntheticNotice={false} appBar={{ title: "Vi um problema", contextLabel: "COMUN Relata" }}>
      <div className="min-h-[calc(100dvh-4rem)] bg-comun-paper px-4 pb-[calc(2rem+env(safe-area-inset-bottom))] pt-6 text-comun-black sm:px-6">
        <main className="mx-auto grid w-full max-w-2xl gap-5" data-comun-quick-capture-v2="true">
          <header className="grid gap-2"><p className="comun-v2-eyebrow">COMUN Relata</p><h1 className="text-3xl font-black leading-tight sm:text-4xl">Vi um problema</h1><p className="text-base leading-7">Uma frase basta. Você pode completar depois.</p></header>
          {!receipt ? <>
            <section className="grid gap-3 border-2 border-comun-black bg-white p-4" aria-labelledby="capture-what"><h2 id="capture-what" className="text-xl font-black">O que aconteceu?</h2><label htmlFor="capture-text" className="text-sm font-bold">Uma frase basta. Não inclua endereço exato, nome ou telefone.</label><textarea id="capture-text" value={text} onChange={(event) => updateText(event.target.value)} rows={4} maxLength={600} className="min-h-28 border-2 border-comun-black p-3 text-base" placeholder="Ex.: a calçada está bloqueada por entulho" /></section>
            {attachmentsEnabled ? <div className="flex flex-wrap gap-3"><label className="inline-flex min-h-11 cursor-pointer items-center border-2 border-comun-black bg-comun-yellow px-4 py-2 text-sm font-black"><input type="file" accept="image/jpeg,image/png,image/webp" capture="environment" className="sr-only" onChange={(event) => { const file = event.target.files?.[0] ?? null; setPhoto(file); setDecision(file ? routeRelata({ text: text.trim().length >= 8 ? text.trim() : "Observação registrada a partir de uma fotografia privada.", hasAttachment: true }) : null); setInteractions((value) => value + 1); }} />{photo ? `Foto pronta (${Math.round(photo.size / 1024)} KB)` : "Tirar ou escolher foto"}</label></div> : null}
          ...
          <aside className="border-2 border-comun-black bg-comun-asphalt p-4 text-sm leading-6 text-comun-paper"><p className="font-black text-comun-yellow">Sem envio automático</p><p>Nenhum órgão público recebeu esta manifestação. Se houver emergência, cuide primeiro da sua segurança.</p>{(attachmentsEnabled || locationEnabled) ? <p className="mt-2">As evidências ativadas continuam privadas e opcionais.</p> : null}<Link href="/comun/relatar?modo=detalhado" className="mt-3 inline-flex underline">Abrir formulário detalhado</Link></aside>
        </main>
      </div>
    </ComunShell>
  );
