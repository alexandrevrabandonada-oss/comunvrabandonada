...
export function QuickCaptureV2({ evidenceEnabled = false }: { evidenceEnabled?: boolean }) {
    ...
    <ComunShell showSyntheticNotice={false} appBar={{ title: "Vi um problema", contextLabel: "COMUN Relata" }}>
      <div className="min-h-[calc(100dvh-4rem)] bg-comun-paper px-4 pb-[calc(2rem+env(safe-area-inset-bottom))] pt-6 text-comun-black sm:px-6">
        <main className="mx-auto grid w-full max-w-2xl gap-5" data-comun-quick-capture-v2="true">
          <header className="grid gap-2"><p className="comun-v2-eyebrow">COMUN Relata</p><h1 className="text-3xl font-black leading-tight sm:text-4xl">Vi um problema</h1><p className="text-base leading-7">Guarde uma frase em menos de um minuto. O registro é privado e ainda não foi encaminhado.</p></header>
          {!receipt ? <>
