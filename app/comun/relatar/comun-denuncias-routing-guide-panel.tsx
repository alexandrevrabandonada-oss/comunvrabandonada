  ...
  return (
    ...
    >
      <div className="grid gap-1">
        <p className="text-xs font-black uppercase">Próximo passo</p>
        <h2 id="denuncias-routing-guide-title" className="text-lg font-black">
          {guide.headline}
        </h2>
        <p className="text-sm leading-6">
          {guide.immediateAction ?? guide.explanation}
        </p>
      </div>
      {guide.primaryChannels.length > 0 ? (
        <GuideBlock
          title={
            guide.primaryChannels.length === 1
              ? "Canal oficial"
              : "Canais disponíveis"
          }
        >
      ...
      <details className="border-t-2 border-comun-black pt-3 text-sm">
        <summary className="min-h-11 cursor-pointer py-2 font-black underline">
          Ver orientações completas
        </summary>
        <div className="grid gap-3 pt-3">
          <p className="leading-6">{guide.explanation}</p>
          ...
          {guide.privacyWarning ? (
            <p className="text-sm">{guide.privacyWarning}</p>
          ) : null}
        </div>
      </details>
