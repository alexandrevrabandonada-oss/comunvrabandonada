...
import { z } from "zod";
import { getComunAdminSession, requireComunAdmin, requireComunAdminRole } from "@/lib/admin-auth";
import { logComunAdminAction } from "@/lib/admin-audit";
import { createAdminNotification, safeDossierNotificationBody, updateAdminNotificationStatus } from "@/lib/admin-notifications";
import { canManagePublicDossierFeatures, canPublishDossier, canReviewEditorial, canReviewFactual, countActiveAdminProfiles, getAdminProfileById, normalizeAdminProfileRole, profileLabel } from "@/lib/admin-profiles";
...
import { createPautaDossierDraft, getAdminPautaDossier, getDossierPublicationSnapshot, regeneratePautaDossierDraft } from "@/lib/pauta-dossiers";
...
export async function updatePautaDossierReviewOpsAction(formData: FormData) {
  ...
  const factualReviewer = String(formData.get("factual_reviewer_assigned") ?? "").trim() || null;
  const editorialReviewer = String(formData.get("editorial_reviewer_assigned") ?? "").trim() || null;
  const factualReviewerUserId = String(formData.get("factual_reviewer_assigned_user_id") ?? "").trim() || null;
  const editorialReviewerUserId = String(formData.get("editorial_reviewer_assigned_user_id") ?? "").trim() || null;
  const factualProfile = factualReviewerUserId ? await getAdminProfileById(factualReviewerUserId) : null;
  const editorialProfile = editorialReviewerUserId ? await getAdminProfileById(editorialReviewerUserId) : null;
  if (factualReviewerUserId && !factualProfile?.active) throw new Error("Responsavel factual precisa ser perfil admin ativo.");
  if (editorialReviewerUserId && !editorialProfile?.active) throw new Error("Responsavel editorial precisa ser perfil admin ativo.");
  ...
  const reviewDueAt = parseOptionalDate(String(formData.get("review_due_at") ?? ""));
  const reviewPriority = normalizeReviewPriority(String(formData.get("review_priority") ?? "normal"));
  const reviewNotesInternal = String(formData.get("review_notes_internal") ?? "").trim() || null;
  const now = new Date().toISOString();
  ...
  if (factualReviewer !== dossier.factual_reviewer_assigned || editorialReviewer !== dossier.editorial_reviewer_assigned || factualReviewerUserId !== dossier.factual_reviewer_assigned_user_id || editorialReviewerUserId !== dossier.editorial_reviewer_assigned_user_id) {
    await logComunAdminAction({
      session,
    ...
    await logComunAdminAction({
      ...
      targetType: "pauta_dossier",
      targetId: dossierId,
      metadata: { factual_bound: Boolean(factualReviewerUserId), editorial_bound: Boolean(editorialReviewerUserId) },
    });
    if (factualLabel && (factualLabel !== dossier.factual_reviewer_assigned || factualReviewerUserId !== dossier.factual_reviewer_assigned_user_id)) {
      const notificationId = await createAdminNotification({
        ...
        body: safeDossierNotificationBody({ priority: reviewPriority, dueAt: reviewDueAt, pendingStage: "Revisao factual" }),
        priority: reviewPriority,
        assignedTo: factualLabel,
        assignedToUserId: factualReviewerUserId,
      });
      await logNotificationCreated(session, notificationId, dossierId, "dossier_factual_assigned");
    }
    if (editorialLabel && (editorialLabel !== dossier.editorial_reviewer_assigned || editorialReviewerUserId !== dossier.editorial_reviewer_assigned_user_id)) {
      const notificationId = await createAdminNotification({
        kind: "dossier_editorial_assigned",
        targetId: dossierId,
        title: "Dossie atribuido para revisao editorial",
        body: safeDossierNotificationBody({ priority: reviewPriority, dueAt: reviewDueAt, pendingStage: "Revisao editorial" }),
        priority: reviewPriority,
        assignedTo: editorialLabel,
        assignedToUserId: editorialReviewerUserId,
      });
      await logNotificationCreated(session, notificationId, dossierId, "dossier_editorial_assigned");
    }
  }
  if (reviewDueAt !== dossier.review_due_at) {
    ...
    if (reviewDueAt) {
      const kind = isBeforeToday(reviewDueAt) ? "dossier_overdue" : isToday(reviewDueAt) ? "dossier_due_today" : "dossier_due_date_changed";
      const notificationId = await createAdminNotification({
        kind,
        targetId: dossierId,
        title: kind === "dossier_overdue" ? "Dossie vencido" : kind === "dossier_due_today" ? "Dossie vence hoje" : "Prazo de revisao alterado",
        body: safeDossierNotificationBody({ priority: reviewPriority, dueAt: reviewDueAt }),
  ...
  if (reviewPriority !== dossier.review_priority) {
    await logComunAdminAction({
      ...
      targetType: "pauta_dossier",
      targetId: dossierId,
      metadata: { previous_priority: dossier.review_priority, next_priority: reviewPriority },
    });
    if (["high", "urgent"].includes(reviewPriority)) {
      const notificationId = await createAdminNotification({
        kind: "dossier_priority_high",
        targetId: dossierId,
        title: reviewPriority === "urgent" ? "Dossie com prioridade urgente" : "Dossie com prioridade alta",
        body: safeDossierNotificationBody({ priority: reviewPriority, dueAt: reviewDueAt }),
        priority: reviewPriority,
        assignedTo: factualLabel || editorialLabel,
        assignedToUserId: factualReviewerUserId || editorialReviewerUserId,
      });
      await logNotificationCreated(session, notificationId, dossierId, "dossier_priority_high");
    }
  }
...
export async function upsertAdminProfileAction(formData: FormData) {
  ...
  const email = String(formData.get("email") ?? "").trim().toLowerCase();
  const role = normalizeAdminProfileRole(String(formData.get("role") ?? "viewer"));
  const active = formData.get("active") === "true";
  const authUserIdInput = String(formData.get("auth_user_id") ?? "").trim();
  const clearAuthLink = formData.get("clear_auth_link") === "true";
  const operationalNote = String(formData.get("operational_note") ?? "").replace(/\s+/g, " ").trim().slice(0, 240) || null;
  if (!displayName || !email) throw new Error("Nome e e-mail sao obrigatorios.");
  const supabase = createServiceSupabaseClient();
  if (!supabase) throw new Error("Supabase service role nao configurado no servidor.");

  const current = profileId ? await getAdminProfileById(profileId) : null;
  const activeAdmins = await countActiveAdminProfiles();
  const wouldRemoveLastAdmin = current?.active && current.role === "admin" && activeAdmins <= 1 && (!active || role !== "admin");
  if (wouldRemoveLastAdmin) {
    ...
    throw new Error("Nao e permitido remover ou desativar o ultimo admin ativo.");
  }
  ...
  const payload = {
    ...
    role,
    active,
    auth_user_id: clearAuthLink ? null : authUserIdInput || current?.auth_user_id || null,
  ...
  await logComunAdminAction({
    ...
    targetType: "admin_profile",
    targetId,
    metadata: { role, active, has_auth_user_id: Boolean(payload.auth_user_id), note_length: operationalNote?.length ?? 0 },
  });
  if (current && current.role !== role) {
    await logComunAdminAction({ session, action: "admin_profile_role_changed", targetType: "admin_profile", targetId, metadata: { previous_role: current.role, next_role: role } });
  }
  if (current && current.active && !active) {
    await logComunAdminAction({ session, action: "admin_profile_deactivated", targetType: "admin_profile", targetId });
  }
  if (current && !current.active && active) {
    await logComunAdminAction({ session, action: "admin_profile_reactivated", targetType: "admin_profile", targetId });
  }
  if (current && current.auth_user_id !== payload.auth_user_id) {
    await logComunAdminAction({ session, action: "admin_profile_auth_link_changed", targetType: "admin_profile", targetId, metadata: { has_auth_user_id: Boolean(payload.auth_user_id) } });
  }
...
export async function savePautaDossierFinalPublicationChecklistAction(formData: FormData) {
  const session = await requireComunAdmin();
  const dossierId = String(formData.get("dossier_id") ?? "");
  if (!dossierId) throw new Error("Dossie sem ID.");
  const checklistValues = formData.getAll("final_publication_checklist").map((value) => String(value));
  const checklist = Object.fromEntries(checklistValues.map((value) => [value, true]));
  const finalNotes = String(formData.get("final_publication_notes") ?? "").trim() || null;
  const supabase = createServiceSupabaseClient();
  if (!supabase) throw new Error("Supabase service role nao configurado no servidor.");
  const { error } = await supabase
    .from("comun_pauta_dossiers")
  ...
  await logComunAdminAction({
    ...
    targetType: "pauta_dossier",
    targetId: dossierId,
    metadata: { checked_count: checklistValues.length, notes_length: finalNotes?.length ?? 0 },
  });
  revalidatePath(`/comun/admin/dossies/${dossierId}`);
  redirect(`/comun/admin/dossies/${dossierId}`);
}

export async function rollbackPautaDossierPublicationSnapshotAction(formData: FormData) {
  const session = await requireComunAdmin();
  ...
  if (!dossier || !source || source.dossier_id !== dossierId) throw new Error("Snapshot nao encontrado para este dossie.");
  const supabase = createServiceSupabaseClient();
  if (!supabase) throw new Error("Supabase service role nao configurado no servidor.");
  const now = new Date().toISOString();
  const active = dossier.publication_snapshots.filter((snapshot) => ["published", "rollback"].includes(snapshot.snapshot_status) && !snapshot.unpublished_at);
  for (const snapshot of active) {
    await supabase.from("comun_pauta_dossier_publication_snapshots").update({ snapshot_status: "superseded" }).eq("id", snapshot.id);
    await logComunAdminAction({
      session,
      action: "dossier_publication_snapshot_superseded",
      targetType: "pauta_dossier_publication_snapshot",
      targetId: snapshot.id,
      metadata: { dossier_id: dossierId, rollback_source_snapshot_id: source.id },
    });
  }
  const { data, error } = await supabase
    ...
    .insert({
      ...
      public_change_note: source.public_change_note || "Versao revisada publicada.",
    ...
    .single();
  if (error) throw new Error(error.message);
  await supabase.from("comun_pauta_dossiers").update({
...
export async function updateDossierPublicationSnapshotPublicNoteAction(formData: FormData) {
  const session = await requireComunAdmin();
  if (!session.profile || !canPublishDossier(session.profile.role)) {
  ...
  if (!snapshot || snapshot.dossier_id !== dossierId) throw new Error("Snapshot nao encontrado para este dossie.");
  const supabase = createServiceSupabaseClient();
  if (!supabase) throw new Error("Supabase service role nao configurado no servidor.");
  const publicChangeNote = sanitizePublicChangeNote(String(formData.get("public_change_note") ?? ""));
  const publicVersionLabel = sanitizePublicVersionLabel(String(formData.get("public_version_label") ?? ""));
  const publicUpdatedAt = parseOptionalDate(String(formData.get("public_updated_at") ?? "")) || snapshot.public_updated_at || snapshot.published_at;
  const { error } = await supabase
    .from("comun_pauta_dossier_publication_snapshots")
    .update({
  ...
  await logComunAdminAction({
    ...
    targetType: "pauta_dossier_publication_snapshot",
    targetId: snapshotId,
    metadata: { dossier_id: dossierId, note_length: publicChangeNote.length, version_label: publicVersionLabel || "Versao revisada" },
  });
  revalidatePath(`/comun/admin/dossies/${dossierId}`);
  revalidatePath("/comun/dossies");
...
export async function updatePautaDossierWorkflowAction(formData: FormData) {
  ...
  if (intent === "send_to_review") {
    ...
    if (!session.profile || !canPublishDossier(session.profile.role)) {
      await logComunAdminAction({ session, action: "review_permission_denied", targetType: "pauta_dossier", targetId: dossierId, metadata: { intent, required_role: "publisher" } });
    ...
    if (!dossier.review_state.canPublish) {
      const action = dossier.review_state.missingReasons.some((reason) => reason.includes("identity"))
      ...
      throw new Error(`Publicacao exige revisao factual e editorial aprovadas por revisores reais distintos vinculados a perfis administrativos: ${dossier.review_state.missingReasons.join(", ")}`);
    }
    const missingFinalChecklist = getMissingFinalPublicationChecklist(dossier.final_publication_checklist);
    if (missingFinalChecklist.length) {
      ...
      throw new Error(`Checklist final incompleto: ${missingFinalChecklist.join(", ")}`);
    }
    ...
    const conflictingSlug = (activeSameSlug.data ?? []).find((snapshot) => snapshot.dossier_id !== dossierId);
    if (conflictingSlug) throw new Error("Ja existe snapshot publico ativo usando este slug.");
    const activeSnapshots = dossier.publication_snapshots.filter((snapshot) => ["published", "rollback"].includes(snapshot.snapshot_status) && !snapshot.unpublished_at);
    for (const snapshot of activeSnapshots) {
      const supersede = await supabase.from("comun_pauta_dossier_publication_snapshots").update({ snapshot_status: "superseded" }).eq("id", snapshot.id);
    ...
    const insertSnapshot = await supabase
      ...
      .insert({
        ...
        public_change_note: sanitizePublicChangeNote(String(formData.get("public_change_note") ?? "")) || (activeSnapshots.length ? "Versao publica revisada." : "Publicacao inicial."),
        public_version_label: sanitizePublicVersionLabel(String(formData.get("public_version_label") ?? "")) || "Versao revisada",
        public_updated_at: now,
      })
      .select("id")
      .single();
    ...
    await logComunAdminAction({
      ...
      metadata: { dossier_id: dossierId, public_slug: dossier.public_slug, previous_active_snapshots: activeSnapshots.length },
    });
    ...
    const unpublishReason = String(formData.get("unpublish_reason") ?? "").trim();
    if (!unpublishReason) throw new Error("Despublicacao exige motivo registrado.");
    const activeSnapshots = dossier.publication_snapshots.filter((snapshot) => ["published", "rollback"].includes(snapshot.snapshot_status) && !snapshot.unpublished_at);
    for (const snapshot of activeSnapshots) {
      ...
      await logComunAdminAction({
        ...
        targetType: "pauta_dossier_publication_snapshot",
        targetId: snapshot.id,
        metadata: { dossier_id: dossierId, reason_length: unpublishReason.length },
      });
    }
  ...
  if (intent === "approve" && dossier.review_state.canPublish) {
    const notificationId = await createAdminNotification({
      ...
      body: safeDossierNotificationBody({ priority: dossier.review_priority, dueAt: dossier.review_due_at, pendingStage: "Pronto para publicar" }),
      priority: dossier.review_priority,
      assignedTo: dossier.editorial_reviewer_assigned || dossier.factual_reviewer_assigned,
      assignedToUserId: dossier.editorial_reviewer_assigned_user_id || dossier.factual_reviewer_assigned_user_id,
    });
    await logNotificationCreated(session, notificationId, dossierId, "dossier_ready_to_publish");
  }
  revalidatePath(`/comun/admin/dossies/${dossierId}`);
  revalidatePath("/comun/admin/dossies");
  revalidatePath("/comun/dossies");
...
export async function createPautaDossierReviewAction(formData: FormData) {
  ...
  if (!profile?.active) {
    await logComunAdminAction({ session, action: "review_permission_denied", targetType: "pauta_dossier", targetId: dossierId, metadata: { review_stage: reviewStage, reason: "missing_active_profile" } });
    throw new Error("Revisao exige perfil admin ativo vinculado ao usuario autenticado.");
  }
  if (decision === "approved" && reviewStage === "factual_review" && !canReviewFactual(profile.role)) {
    await logComunAdminAction({ session, action: "review_permission_denied", targetType: "pauta_dossier", targetId: dossierId, metadata: { review_stage: reviewStage, role: profile.role } });
    throw new Error("Revisao factual exige perfil factual_reviewer, editor ou admin.");
  }
  if (decision === "approved" && reviewStage === "editorial_review" && !canReviewEditorial(profile.role)) {
    await logComunAdminAction({ session, action: "review_permission_denied", targetType: "pauta_dossier", targetId: dossierId, metadata: { review_stage: reviewStage, role: profile.role } });
    throw new Error("Revisao editorial exige perfil editorial_reviewer, editor ou admin.");
  }
  const dossier = await getAdminPautaDossier(dossierId);
  if (!dossier) throw new Error("Dossie nao encontrado.");
  if (decision === "approved" && reviewStage === "factual_review" && dossier.review_state.editorialReviewerUserId === profile.id) {
    await logComunAdminAction({ session, action: "review_same_user_blocked", targetType: "pauta_dossier", targetId: dossierId, metadata: { review_stage: reviewStage } });
    throw new Error("A mesma conta nao pode aprovar factual e editorial do mesmo dossie.");
  }
  if (decision === "approved" && reviewStage === "editorial_review" && dossier.review_state.factualReviewerUserId === profile.id) {
    await logComunAdminAction({ session, action: "review_same_user_blocked", targetType: "pauta_dossier", targetId: dossierId, metadata: { review_stage: reviewStage } });
    throw new Error("A mesma conta nao pode aprovar factual e editorial do mesmo dossie.");
  }
  ...
  await logComunAdminAction({
    ...
    targetType: "pauta_dossier_review",
    targetId: data.id,
    metadata: { dossier_id: dossierId, reviewer_user_id: profile.id, review_stage: reviewStage },
  });
  ...
  if (updatedDossier) {
    if (decision === "changes_requested") {
      const notificationId = await createAdminNotification({
        ...
        body: safeDossierNotificationBody({ priority: updatedDossier.review_priority, dueAt: updatedDossier.review_due_at, pendingStage: "Ajustes solicitados" }),
        priority: updatedDossier.review_priority,
        assignedTo: updatedDossier.editorial_reviewer_assigned || updatedDossier.factual_reviewer_assigned,
        assignedToUserId: updatedDossier.editorial_reviewer_assigned_user_id || updatedDossier.factual_reviewer_assigned_user_id,
      });
      await logNotificationCreated(session, notificationId, dossierId, "dossier_changes_requested");
    }
    if (updatedDossier.review_state.factualApproved && updatedDossier.review_state.editorialApproved && !updatedDossier.review_state.reviewersDistinct) {
      const notificationId = await createAdminNotification({
        kind: "dossier_blocked_same_reviewer",
        targetId: dossierId,
        title: "Dossie bloqueado por mesmo revisor",
        body: safeDossierNotificationBody({ priority: updatedDossier.review_priority, dueAt: updatedDossier.review_due_at, pendingStage: "Revisor distinto necessario" }),
        priority: "high",
        assignedTo: updatedDossier.editorial_reviewer_assigned || updatedDossier.factual_reviewer_assigned,
        assignedToUserId: updatedDossier.editorial_reviewer_assigned_user_id || updatedDossier.factual_reviewer_assigned_user_id,
      });
      await logNotificationCreated(session, notificationId, dossierId, "dossier_blocked_same_reviewer");
    }
    if (updatedDossier.review_state.canPublish && updatedDossier.review_status === "approved" && updatedDossier.public_title && updatedDossier.public_summary && updatedDossier.public_body && updatedDossier.public_slug) {
      const notificationId = await createAdminNotification({
        kind: "dossier_ready_to_publish",
        targetId: dossierId,
        title: "Dossie pronto para publicacao",
        body: safeDossierNotificationBody({ priority: updatedDossier.review_priority, dueAt: updatedDossier.review_due_at, pendingStage: "Pronto para publicar" }),
        priority: updatedDossier.review_priority,
        assignedTo: updatedDossier.editorial_reviewer_assigned || updatedDossier.factual_reviewer_assigned,
        assignedToUserId: updatedDossier.editorial_reviewer_assigned_user_id || updatedDossier.factual_reviewer_assigned_user_id,
      });
...
export async function upsertPublicDossierFeatureAction(formData: FormData) {
  ...
  if (!profile?.active || !canManagePublicDossierFeatures(profile.role)) {
    ...
    throw new Error("Destaque publico exige perfil admin, editor ou publisher.");
  }
  const dossierId = String(formData.get("dossier_id") ?? "");
  const snapshotId = String(formData.get("snapshot_id") ?? "");
  if (!dossierId || !snapshotId) throw new Error("Destaque sem dossie ou snapshot.");
  const publicLabel = String(formData.get("public_label") ?? "").trim().slice(0, 80) || null;
  const publicNote = String(formData.get("public_note") ?? "").trim().slice(0, 280) || null;
  ...
  const { data: snapshot, error: snapshotError } = await supabase
    ...
    .maybeSingle();
  if (snapshotError || !snapshot) throw new Error("Snapshot nao encontrado para este dossie.");
  ...
  const result = existing?.id
    ? await supabase.from("comun_public_dossier_features").update(payload).eq("id", existing.id).select("id").single()
    : await supabase.from("comun_public_dossier_features").insert(payload).select("id").single();
  if (result.error) throw new Error(result.error.message);
  await logComunAdminAction({
    session,
    action: active ? "public_dossier_feature_updated" : "public_dossier_feature_disabled",
    targetType: "public_dossier_feature",
  ...
  revalidatePath("/comun/dossies");
  revalidatePath(`/comun/admin/dossies/${dossierId}`);
  if (snapshot.public_slug) revalidatePath(`/comun/dossies/${snapshot.public_slug}`);
  redirect(`/comun/admin/dossies/${dossierId}`);
}
...
function normalizeReviewPriority(value: string): PautaDossierReviewPriority {
  ...
  return valid.includes(value) ? (value as PautaDossierReviewPriority) : "normal";
}
...
async function logNotificationCreated(session: Awaited<ReturnType<typeof requireComunAdmin>>, notificationId: string | null, dossierId: string, kind: string) {
...
function getMissingFinalPublicationChecklist(checklist: Record<string, boolean> | null | undefined) {
  const required = [
    "title_reviewed",
    "summary_reviewed",
    "body_reviewed",
...
function sanitizePublicChangeNote(value: string) {
  return value
    .replace(/\b(raw_text|private_contact|internal_notes|response_text|storage_path|signed_url|rollback|auditoria|checklist|revisor|reviewer|publisher)\b/gi, "")
    .replace(/\s+/g, " ")
    .trim()
    .slice(0, 220);
}
