...
A terceira execução fechou contraste, foco, formulário, sessão expirada e 18/20 cenários. Os dois resíduos eram o mesmo teste de retorno: a simulação por `history.replaceState` alterava a URL fora do ciclo inicial do App Router e não representava a entrada real por deep link. A tentativa seguinte confirmou que uma segunda navegação server-side para a mesma rota, variando somente a query string, permanece no overlay `Rendering…` do Next dev até o timeout. O cenário focal passou a verificar diretamente o contrato de deep link nos extremos 390×844 e 1366×768: `returnTo` filtrado, sanitização, flag V2 e href contextual de volta. A captura automática do allowlist permanece coberta no teste unitário. O produto não ganhou estado paralelo para satisfazer o teste.
