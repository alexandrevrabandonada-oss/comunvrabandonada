...
- O validador lê `pre.bundleLedgerState` do novo artifact, confere que todos os campos anteriores permanecem byte-equivalentes semanticamente ao PRE de hash pinado e classifica `PRE` somente com `ABSENT`. `PRESENT_ACCEPTED` com R1/R2 ausentes, `PRESENT_MISMATCH` e qualquer formato inesperado bloqueiam. O manifest e seus hashes/fingerprints esperados permanecem inalterados.
