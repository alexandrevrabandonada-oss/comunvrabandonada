...
describe("comunidades vivas",()=>{
 ...
 it("filtra sem popularidade",()=>{expect(filterCommunityExperiences([{...getCommunityExperience("trabalho")!,slug:"cidade",purpose:"calçada sintética"}],"calçada","","",false).map(x=>x.slug)).toEqual(["cidade"]);expect(filterCommunityExperiences(listCommunityExperiences(),"","territorial","",false).every(x=>x.kind==="territorial")).toBe(true)});
