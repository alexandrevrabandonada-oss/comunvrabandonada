...
import { isComunBusLocalPilotEnabled, isComunBusRelataEnabled } from "./comun-bus-feature";
...
export function createComunBusRelataClient(env: Record<string, string | undefined> = process.env) {
  if (!isComunBusRelataEnabled(env)) throw new Error("COMUN_BUS_RELATA_RUNTIME_REQUIRED");
  return createClient(env.NEXT_PUBLIC_SUPABASE_URL!, env.SUPABASE_SERVICE_ROLE_KEY!, {
    auth: { persistSession: false, autoRefreshToken: false },
    global: { headers: { "x-comun-scope": "bus-relata-server-v1" } },
  });
}
