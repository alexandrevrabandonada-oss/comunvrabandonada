...
describe("COMUN Relata evidence flags", () => {
  ...
  it("fails before reading cryptographic secrets when a flag is off", () => {
    let secretReads = 0;
    const env: Record<string, string | undefined> = { ...enabledEnv(), COMUN_RELATA_LOCAL_EVIDENCE: "disabled" };
    Object.defineProperty(env, COMUN_RELATA_LOCATION_KEY, { get() { secretReads += 1; return key(1); } });
    Object.defineProperty(env, COMUN_RELATA_SPATIAL_KEY, { get() { secretReads += 1; return key(2); } });
  ...
  it("cloaks every evidence subroute before method dispatch while dormant", () => {
    const dormant = { ...enabledEnv(), COMUN_RELATA_LOCAL_EVIDENCE: "disabled" };
    expect(shouldCloakComunRelataEvidenceApi("/api/comun/relata/evidence/location", dormant)).toBe(true);
    expect(shouldCloakComunRelataEvidenceApi("/api/comun/relata/evidence/grouping", dormant)).toBe(true);
    expect(shouldCloakComunRelataEvidenceApi("/api/comun/relata/evidence-not-this-contract", dormant)).toBe(false);
    expect(shouldCloakComunRelataEvidenceApi("/api/comun/relata/evidence/location", enabledEnv())).toBe(false);
  });
  it("requires all evidence barriers plus the explicit collective flag", () => {
    const env = { ...enabledEnv(), COMUN_RELATA_COLLECTIVE_ENABLED: "enabled" };
    expect(isComunRelataCollectiveEnabled(env)).toBe(true);
    expect(isComunRelataCollectiveEnabled({ ...env, COMUN_RELATA_COLLECTIVE_ENABLED: "disabled" })).toBe(false);
    expect(isComunRelataCollectiveEnabled({ ...env, COMUN_RELATA_LOCAL_EVIDENCE: "disabled" })).toBe(false);
  });
});
