...
function storageConfig(allowRemote=false){const url=process.env.NEXT_PUBLIC_SUPABASE_URL||"",key=process.env.SUPABASE_SERVICE_ROLE_KEY||"",local=/^https?:\/\/(localhost|127\.0\.0\.1)(:\d+)?$/i.test(url),remote=/^https:\/\/[a-z0-9]+\.supabase\.co$/i.test(url);if(!local&&!(allowRemote&&remote))throw new Error("Supabase Storage exige destino local ou remoto explicitamente autorizado.");if(!key)throw new Error("Service role do storage ausente.");return{url,key}}
...
export class SupabaseLocalStorageProvider implements MediaStorageProvider{private db:SupabaseClient;private url:string;constructor(options:{allowRemote?:boolean}={}){const c=storageConfig(options.allowRemote);this.url=c.url;this.db=createClient(c.url,c.key,{auth:{persistSession:false}})}
