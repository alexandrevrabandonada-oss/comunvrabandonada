...
export async function listPublicArtworks(options: { query?: string; type?: string; territoryId?: string; territorySlug?: string; page?: number; limit?: number } = {}) {
  ...
  if(!territoryId&&options.territorySlug){const{data:territory}=await db.from("comun_hub_territories" as never).select("id" as never).eq("slug" as never,options.territorySlug).eq("visibility" as never,"public").maybeSingle();if(!territory)return{items:[],count:0};territoryId=(territory as any).id}
  let query = db.from("comun_archive_items" as never).select("id,slug,title,summary,published_at,comun_archive_artworks!inner(artwork_type,title_public,description_public,context_public,creation_year,creation_period_public,technique_public,materials_public,territory_id,long_description_public,territory:comun_hub_territories(id,slug,name,visibility)),comun_archive_assets(id,asset_role,public_url,alt_text,credits,width,height),comun_archive_artwork_credits(public_credit,credit_role,position),comun_archive_artwork_rights(required_credit_public,license_public,allow_download,allow_social_media)" as never,{count:"exact"}).eq("item_type" as never,"territorial_artwork").eq("status" as never,"published").eq("visibility" as never,"public").order("published_at" as never,{ascending:false}).range((page-1)*limit,page*limit-1);
  if (options.query) query = query.or(`title.ilike.%${options.query.replaceAll(",","")}%,summary.ilike.%${options.query.replaceAll(",","")}%` as never); if (options.type) query = query.eq("comun_archive_artworks.artwork_type" as never,options.type);if(territoryId)query=query.eq("comun_archive_artworks.territory_id" as never,territoryId);
  const { data, count } = await query; return { items: (data || []) as any[], count: count || 0 };
}

export async function listPublicArtworkTerritories(){const db=createServiceSupabaseClient();if(!db)return[];const{data}=await db.from("comun_hub_territories" as never).select("id,slug,name" as never).eq("visibility" as never,"public").order("name" as never).limit(200);return(data||[])as any[]}

export async function getPublicArtwork(slug: string) { const db=createServiceSupabaseClient(); if(!db)return null; const {data}=await db.from("comun_archive_items" as never).select("id,slug,title,summary,description,neighborhood,approximate_date,published_at,comun_archive_artworks!inner(*,territory:comun_hub_territories(id,slug,name,visibility)),comun_archive_assets(id,asset_role,public_url,alt_text,credits,width,height),comun_archive_artwork_credits(public_credit,credit_role,position,agent:comun_archive_agents(public_name,public_slug,public_bio,public_visibility,status)),comun_archive_artwork_rights(required_credit_public,license_public,allow_download,allow_social_media,allow_print,allow_exhibition,allow_educational_use,allow_campaign_use),comun_archive_artwork_relations(relation_type,target_type,target_id,public_note)" as never).eq("slug" as never,slug).eq("item_type" as never,"territorial_artwork").eq("status" as never,"published").eq("visibility" as never,"public").maybeSingle(); return data as any; }
