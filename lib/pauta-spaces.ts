...
export type PublicPautaContribution = Pick<
  PautaContribution,
  | "id"
  | "pauta_id"
  | "contribution_type"
  ...
  | "created_at"
>;
export type PublicPautaTask = Pick<
  PautaTask,
  | "id"
  | "pauta_id"
  | "title"
  ...
  | "created_at"
>;
export type PublicPautaEvidenceItem = Pick<
  PautaEvidenceItem,
  | "id"
  | "pauta_id"
  | "title"
...
export async function getPublicPautaSpaceBySlug(slug: string) {
  const supabase = createServiceSupabaseClient();
  if (!supabase)
    return slug === CANONICAL_SIDEWALK_PAUTA_SLUG
      ? canonicalSidewalkEditorialPauta
      : null;
...
export async function listAdminPautaSpaces() {
  ...
  const { data, error } = await supabase
    .from("comun_pauta_spaces")
    .select(
      "id, slug, title, summary, category, community, status, visibility, public_synthesis, next_step, created_from_signal, editorial_checklist, public_status, priority, urgency, risk_level, responsible_internal, responsible_public, territory_id, affected_people_public, problem_public, demand_public, proposals_public, participation_public, last_operational_update_at, created_at, updated_at",
    )
    .order("updated_at", { ascending: false });
...
export async function getAdminPautaSpace(id: string) {
  ...
  const { data, error } = await supabase
    .from("comun_pauta_spaces")
    .select(
      "id, slug, title, summary, category, community, status, visibility, public_synthesis, next_step, created_from_signal, editorial_checklist, public_status, internal_status, priority, urgency, risk_level, responsible_internal, responsible_public, territory_id, affected_people_public, problem_public, demand_public, proposals_public, participation_public, last_operational_update_at, created_at, updated_at",
    )
    .eq("id", id)
    .limit(1)
    .maybeSingle();
...
export async function listAdminPautaContributions(pautaId: string) {
  ...
  const { data, error } = await supabase
    .from("comun_pauta_contributions")
    .select(
      "id, pauta_id, contribution_type, author_alias, body, contact_private, status, risk_level, risk_reasons, moderation_priority, submitter_hash, user_agent_hash, reviewed_at, reviewed_by, moderator_notes, created_at, updated_at",
    )
    .eq("pauta_id", pautaId)
    .order("created_at", { ascending: false });
...
export async function listAdminPautaContributionQueue(
  filters: {
    status?: string;
    riskLevel?: string;
  ...
  if (!supabase)
    return [] as Array<
      PautaContribution & {
        pauta: Pick<PautaSpace, "id" | "slug" | "title"> | null;
      }
    >;
  ...
  let query = supabase
    .from("comun_pauta_contributions")
    .select(
      "id, pauta_id, contribution_type, author_alias, body, contact_private, status, risk_level, risk_reasons, moderation_priority, submitter_hash, user_agent_hash, reviewed_at, reviewed_by, moderator_notes, created_at, updated_at, pauta:comun_pauta_spaces(id, slug, title)",
    )
...
export async function listPautaSynthesisVersions(pautaId: string) {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, pauta_id, previous_public_synthesis, new_public_synthesis, previous_next_step, new_next_step, editor_note, created_at",
    )
    .eq("pauta_id", pautaId)
    .order("created_at", { ascending: false })
    .limit(10);
...
export async function listAdminPautaTasks(pautaId: string) {
  ...
  const { data, error } = await supabase
    .from("comun_pauta_tasks")
    .select(
      "id, pauta_id, title, description, status, help_needed, owner_alias, due_at, action_id, project_id, required_skill, priority, visibility, accepts_volunteers, participant_limit, result_public, created_at, updated_at",
    )
    .eq("pauta_id", pautaId)
    .order("created_at", { ascending: false });
...
export async function listSafePautaReports(
  space: Pick<PautaSpace, "category" | "community">,
) {
  const supabase = createServiceSupabaseClient();
  if (!supabase) return [];
  ...
  let query = supabase
    .from("comun_reports")
    .select(
      "id, protocol, community_slug, issue_slug, title, public_text, period_text, approximate_location, neighborhood, created_at, published_at",
    )
...
export async function listSafePautaOfficialProtocols(
  space: Pick<PautaSpace, "category" | "community">,
) {
  const supabase = createServiceSupabaseClient();
  if (!supabase) return [];
  ...
  let query = supabase
    ...
    .select(
      "id, comun_protocol, channel, agency, official_protocol_number, submitted_at, expected_response_at, status, response_received_at, satisfaction, public_summary, updated_at, report:comun_reports!inner(community_slug, issue_slug)",
    )
    .limit(50)
    .order("updated_at", { ascending: false });

  if (space.community)
    query = query.eq("comun_reports.community_slug", space.community);
  if (space.category)
    query = query.eq("comun_reports.issue_slug", space.category);
...
export async function assessPautaContributionSafety(input: {
  ...
  const bodyHash = hashLookupValue(
    input.body.toLowerCase().replace(/\s+/g, " ").slice(0, 500),
  );
  ...
  if ((input.body.match(/https?:\/\//gi) ?? []).length >= 2)
    riskReasons.push("muitos_links");
  ...
  if (input.challengeAnswer.trim() !== "5")
    riskReasons.push("desafio_invalido");
  if (bodyHash && (await hasRepeatedRecentBody(input.pautaId, bodyHash)))
    riskReasons.push("texto_repetido");
  ...
  const hasHardSignal =
    riskReasons.includes("honeypot_preenchido") ||
    riskReasons.includes("desafio_invalido");
...
async function withPautaStats(space: PautaSpace): Promise<PublicPautaSpace> {
  ...
  return {
    ...
    stats: {
      ...
      overdueProtocolCount: protocols.filter(
        (protocol) => protocol.timing.isOverdue,
      ).length,
      waitingResponseCount: protocols.filter(
        (protocol) => protocol.status === "waiting_response",
      ).length,
      resolvedCount: protocols.filter((protocol) =>
        ["resolved", "satisfactory_response"].includes(protocol.status),
      ).length,
      unresolvedCount: protocols.filter((protocol) =>
        ["unresolved", "unsatisfactory_response"].includes(protocol.status),
      ).length,
      pendingContributionCount: contributions.filter(
        (contribution) => contribution.status === "pending",
      ).length,
      openTaskCount: tasks.filter((task) =>
...
function containsOffensiveTerm(body: string) {
  ...
  return ["idiota", "burro", "otario", "lixo humano"].some((term) =>
    text.includes(term),
  );
}
