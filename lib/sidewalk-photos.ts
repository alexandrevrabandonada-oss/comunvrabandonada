...
export async function createFixtureSidewalkImage(): Promise<{ buffer: Buffer; width: number; height: number; mime: string }> {
  const width = 800;
  const height = 600;
  ...
  return { buffer, width: meta.width ?? width, height: meta.height ?? height, mime: "image/jpeg" };
}

export async function validateSidewalkPhotoImage(body: Uint8Array, filename: string) {
  ...
  if (!(allowed.get(mime) ?? []).some((sig) => sig.every((x, i) => body[i] === x)) || (mime === "image/webp" && Buffer.from(body.slice(8, 12)).toString() !== "WEBP")) {
...
export async function createSidewalkPhotoUploadTarget(itemId: string, filename: string, sizeBytes: number) {
  const provider = getMediaStorage();
  const key = `originals/sidewalk/${itemId}/${filename}`;
  return provider.createUploadTarget({ scope: "private_original", key, contentType: "image/jpeg", sizeBytes });
}
...
export async function confirmAndStoreSidewalkOriginal(
  ...
  const { data, error } = await db.from("comun_archive_assets" as never).insert({
...
export async function generateSidewalkPhotoDerivative(
  ...
  if (error) throw error;
  return { assetId: (data as any).id, key, publicUrl, width: meta.width, height: meta.height, size: derivative.byteLength };
}

export async function createSidewalkPhotoItem(db: SupabaseClient, title: string, slug: string) {
  const { data, error } = await db.from("comun_archive_items" as never).insert({
...
export async function createSidewalkRecordPhoto(
  ...
  const { data, error } = await db.from("comun_sidewalk_record_photos" as never).insert({
    record_id: recordId,
    archive_item_id: archiveItemId,
    original_asset_id: originalAssetId,
    derivative_asset_id: derivativeAssetId,
    review_status: "approved",
    checklist: Object.fromEntries(PHOTO_REVIEW_CHECKLIST.map((k) => [k, false])),
    is_public: true,
...
export async function cleanupSidewalkPhotoAssets(db: SupabaseClient, prefixItemId: string) {
  const provider = getMediaStorage();
  const keys: string[] = [];
  for (const scope of ["private_original", "public_safe"] as const) {
    const prefix = scope === "private_original" ? `originals/sidewalk/${prefixItemId}` : `public/sidewalk/${prefixItemId}`;
