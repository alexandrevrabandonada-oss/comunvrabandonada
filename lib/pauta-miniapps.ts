import { pautaAppTemplates, pautaModuleRegistry, type PautaModuleType, validatePautaModuleConfig } from "@/lib/comun/pauta-module-registry";
...
export async function listPublicPautaModules(pautaId: string) {
  ...
  const { data, error } = await supabase
    ...
    .select("id, pauta_id, module_type, title_override, public_description, position, config" as never)
    .eq("pauta_id" as never, pautaId)
    .eq("status" as never, "active")
    .eq("visibility" as never, "public")
    .order("position" as never, { ascending: true });
  if (error || !data) return [];
  return (data as unknown as PublicPautaModule[]).filter((module) => module.module_type in pautaModuleRegistry);
}

export async function listAdminPautaModules(pautaId: string) {
  const supabase = createServiceSupabaseClient();
  if (!supabase) return [] as Array<PublicPautaModule & { status: string; visibility: string }>;
  const { data, error } = await supabase
    .from("comun_pauta_modules" as never)
    .select("id, pauta_id, module_type, title_override, public_description, position, status, visibility, config" as never)
    .eq("pauta_id" as never, pautaId)
    .order("position" as never, { ascending: true });
  return error || !data ? [] : (data as unknown as Array<PublicPautaModule & { status: string; visibility: string }>);
}
...
export async function applyPautaAppTemplate(pautaId: string, template: keyof typeof pautaAppTemplates, createdBy: string) {
  const supabase = createServiceSupabaseClient();
  ...
  const rows = modules.filter((type) => !current.has(type)).map((moduleType, position) => ({
    pauta_id: pautaId,
  ...
  const { error } = await supabase.from("comun_pauta_modules" as never).insert(rows as never);
  if (error) throw new Error(error.message);
  return { created: rows.length, skipped: modules.length - rows.length };
}

export async function upsertPautaModule(input: {
  pautaId: string; moduleType: PautaModuleType; title: string; description: string; position: number; status: string; visibility: string; configText: string; createdBy: string;
}) {
  const config = JSON.parse(input.configText || "{}");
  const validation = validatePautaModuleConfig(input.moduleType, config);
  if (!validation.success) throw new Error("Configuração inválida para este módulo.");
  if (input.position < 0) throw new Error("A posição deve ser positiva.");
  if (input.moduleType.endsWith("_future") && input.visibility === "public") throw new Error("Módulos futuros permanecem internos nesta sprint.");
  const supabase = createServiceSupabaseClient();
  if (!supabase) throw new Error("Supabase service role nao configurado no servidor.");
  const { error } = await supabase.from("comun_pauta_modules" as never).upsert({
    pauta_id: input.pautaId, module_type: input.moduleType, title_override: input.title || null, public_description: input.description || null,
    position: input.position, status: input.status, visibility: input.visibility, config: validation.data, created_by: input.createdBy, updated_at: new Date().toISOString(),
  } as never, { onConflict: "pauta_id,module_type" as never });
  if (error) throw new Error(error.message);
}
...
export async function listMyParticipation(userId: string) {
  ...
  const [memberships, contributions, artworkSubmissions, radioContributions, sidewalkRecords] = await Promise.all([
    ...
    supabase.from("comun_sidewalk_records" as never).select("id,status,verification_status,forwarding_status,created_at,updated_at,last_observed_at,approximate_location,pauta:comun_pauta_spaces(title,slug)" as never).eq("member_user_id" as never,userId).order("created_at" as never,{ascending:false}),
  ]);
  const sidewalkRows=(sidewalkRecords.data??[]) as any[],recordIds=sidewalkRows.map((x:any)=>x.id);const{data:priorities}=recordIds.length?await supabase.from("comun_sidewalk_priorities" as never).select("id,record_id" as never).in("record_id" as never,recordIds).eq("status" as never,"approved"):{data:[]};const priorityIds=(priorities??[] as any[]).map((x:any)=>x.id),{data:forwardings}=priorityIds.length?await supabase.from("comun_sidewalk_forwardings" as never).select("priority_id,state,title_public,updated_at" as never).in("priority_id" as never,priorityIds).neq("state" as never,"archived"):{data:[]};const priorityByRecord=new Map((priorities??[] as any[]).map((x:any)=>[x.record_id,x.id])),forwardingByPriority=new Map((forwardings??[] as any[]).map((x:any)=>[x.priority_id,x]));const sidewalk=sidewalkRows.map((item:any)=>{const flow:any=forwardingByPriority.get(priorityByRecord.get(item.id)),operational=projectSidewalkOperationalState({...item,forwarding_state:flow?.state,updated_at:flow?.updated_at??item.updated_at});return{...item,title_suggestion:item.pauta?.title??"Pauta das calçadas",status:operational.state,next_action_public:operational.nextAction,last_changed_at:operational.lastChangedAt,action_url:item.pauta?.slug?`/comun/pautas/${item.pauta.slug}`:"/comun/calcadas"}});
