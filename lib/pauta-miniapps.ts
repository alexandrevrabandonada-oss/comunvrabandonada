...
export async function listMyParticipation(userId: string) {
  ...
  const sidewalkRows=(sidewalkRecords.data??[]) as any[],recordIds=sidewalkRows.map((x:any)=>x.id);const{data:priorities}=recordIds.length?await supabase.from("comun_sidewalk_priorities" as never).select("id,record_id" as never).in("record_id" as never,recordIds).eq("status" as never,"approved"):{data:[]};const priorityIds=(priorities??[] as any[]).map((x:any)=>x.id),{data:forwardings}=priorityIds.length?await supabase.from("comun_sidewalk_forwardings" as never).select("priority_id,state,title_public" as never).in("priority_id" as never,priorityIds).neq("state" as never,"archived"):{data:[]};const priorityByRecord=new Map((priorities??[] as any[]).map((x:any)=>[x.record_id,x.id])),forwardingByPriority=new Map((forwardings??[] as any[]).map((x:any)=>[x.priority_id,x]));const sidewalk=sidewalkRows.map((item:any)=>{const flow:any=forwardingByPriority.get(priorityByRecord.get(item.id));return{...item,title_suggestion:item.pauta?.title??"Pauta das calçadas",next_action_public:flow?`Seu registro integra esta prioridade e este encaminhamento: ${flow.title_public}. Estado: ${flow.state}.`:`${item.approximate_location??"Local aproximado"} · aguardar revisão. Volte à pauta para acompanhar o processo.`}});
