...
import { createRelataPreview, sanitizeRelataLogEvent } from "./comun-relata-preview";
...
describe("COMUN Relata local preview", () => {
  it("creates a COMUN-only protocol without official forwarding", () => {
    ...
    const preview = createRelataPreview({ text: "A luminária da rua está apagada" }, decision);
  ...
  it("keeps PII out of preview logs", () => {
    const clean = sanitizeRelataLogEvent({ category: "public_lighting", text: "email pessoa@example.com", latitude: -22 });
